...
- **Dode orphans — niet als balans gebruiken:** `profiles.credits` (`DEFAULT 5`) heeft **geen callers** (de oude SQL-functie `deduct_credits(uuid,integer,text,jsonb)` die die kolom muteerde is op 2026-09-02 **gedropt** in de security-audit, migratie `20260901221134`; net als `reset_monthly_quota`). Idem ongebruikt: `credit_transactions.balance_after`/`transaction_type`, `user_credits.total_credits_purchased`/`credits_bonus`.
