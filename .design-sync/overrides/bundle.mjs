// forked from design-sync lib/bundle.mjs — add process.env shim so Next.js process.env.__NEXT_* refs don't throw in browser
...
function sharedBuildOptions({ nodePaths, tsconfig }) {
  ...
  return {
    ...
    minify: false,
    // 'process.env' shim: Next.js uses process.env.__NEXT_* which is not
    // replaced by the NODE_ENV define and throws in the browser IIFE.
    // More-specific defines (process.env.NODE_ENV) still win over this.
