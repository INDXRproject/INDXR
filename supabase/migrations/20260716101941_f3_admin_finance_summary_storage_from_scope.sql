-- F3: admin_finance_summary reads storage COR from _geld_scope (now per-user attributed) instead of adding
-- the full storage flat against revenue. Drops `v_cor_rev := v_cor_rev + v_cor_sto` (storage already inside
-- rec.cor_against_revenue and rec.granted_delivery_cost). Exposes storage_approx for the UI marker.
...
BEGIN
  ...
  FOR v_scope, v_internal IN SELECT s, i FROM (VALUES ('external', false), ('internal', true)) AS t(s, i) LOOP
    ...
    v_cor_rev := (rec->>'cor_against_revenue')::numeric;   -- F3: already includes per-user storage
    v_good    := (rec->>'granted_delivery_cost')::numeric; -- F3: already includes storage goodwill
    v_cor_sto := (rec#>>'{cor,storage}')::numeric;         -- F3: storage COR from _geld_scope (per-user)
