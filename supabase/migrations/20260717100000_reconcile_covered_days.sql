-- Point-1 fix: the Decodo reconciliation compared measured proxy bytes over the FULL selected period against
-- billed bytes that only exist for the days actually fetched → a "This month" view measured 16 days vs 2 billed
-- days and produced a false NEGATIVE gap (-0.025 GB). Fix: restrict measured to the days that have a
-- decodo_daily_usage row (the covered days), so billed and measured span the same days. The worker now writes a
-- row for every fetched day (0 when the day had no traffic), so "row exists" = "day covered" — unambiguous.
-- Also expose data_from (earliest Decodo day) so the UI can say "gap over N of M days · Decodo data starts …".
-- Only the reconciliation block + two declarations + the reconciliation jsonb change; every formula is verbatim.
...
BEGIN
  ...
  -- Decodo reconciliation (account-level, scope-independent). Compare LIKE-FOR-LIKE: measured proxy bytes only
  -- on the days Decodo actually reports (covered days = days with a decodo_daily_usage row). This kills the
  -- false negative gap that appeared when the picked period was wider than the fetched days.
  v_cov_day_arr := ARRAY(SELECT day FROM public.decodo_daily_usage WHERE day >= v_from_d AND day < v_to_d);
  SELECT COALESCE(sum(billed_bytes),0), count(*), min(day)
    INTO v_billed_bytes, v_cov_days, v_data_from
  ...
  IF v_cov_days = 0 THEN
    v_measured_bytes := 0; v_recon_status := 'unavailable'; v_recon_gap_bytes := 0; v_recon_gap_cost := 0;
  ELSE
    -- Measured = ALL our proxy bytes across both scopes (Decodo bills one account; test+prod share one proxy
    -- user → not splittable), restricted to the covered days (UTC-day membership, matching Decodo's daily key).
    SELECT
    ...
    -- Book only a POSITIVE gap (Decodo billed more than we counted = genuinely uncounted cost). A negative gap
    -- (we measured decompressed bodies > Decodo's wire bytes) is shown but not clawed back.
  ...
  FOR v_scope, v_internal IN SELECT s, i FROM (VALUES ('external', false), ('internal', true)) AS t(s, i) LOOP
    ...
    -- Reconciliation gap counts in the EXTERNAL P&L only (account-level truth → real economy).
    IF v_internal = false THEN
