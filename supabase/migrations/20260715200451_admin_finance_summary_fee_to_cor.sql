-- admin_finance_summary: F22 (Stripe-fee van OPEX naar COR, mee-deferrend) + F1 (per-user against_revenue_by_method
-- uit _geld_scope i.p.v. v_share) + F1b (echte deferred_credits + deferred_fee).
--  * measured_opex bevat GEEN stripe_fee meer (verhuisd naar COR via cor_against_revenue/recognized_fee).
--  * cor.against_revenue = _geld_scope.cor_against_revenue (measured against-rev + recognized_fee) + storage.
--  * against_revenue_by_method komt per-user uit _geld_scope + storage + payment_fee (geen v_share-herrekening).
--  * cor.payment_fee = {recognized, deferred, purchased, by_type}; deferred_fee ook in de Deferred-kaart.
--  * bank.stripe_fee blijft de ECHTE cash-fee (charged − fee = settled) — dat is een cashflow-feit, geen P&L-kost.
--  * deferred_credits = echte Σ lot_rem (F1b), niet terugrekenen uit een blended gemiddelde.
...
BEGIN
  ...
  FOR v_scope, v_internal IN SELECT s, i FROM (VALUES ('external', false), ('internal', true)) AS t(s, i) LOOP
    ...
    v_cor_rev := (rec->>'cor_against_revenue')::numeric;   -- bevat al recognized_fee (F22) + per-user measured (F1)
    ...
    v_defer_credits := (rec->>'deferred_credits')::numeric;   -- F1b echte Σ lot_rem
    ...
    v_cor_meas_total := v_cor_ai + v_cor_cap + v_cor_sum + v_cor_rag + v_cor_sto;  -- VOLLE measured COR (COR-tabel)
    v_cor_rev := v_cor_rev + v_cor_sto;                                            -- against-revenue: + storage
    v_gross := v_revdel - v_cor_rev;
    -- OPEX: GEEN stripe_fee meer (verhuisd naar COR). Goodwill/funnel/radar blijven.
