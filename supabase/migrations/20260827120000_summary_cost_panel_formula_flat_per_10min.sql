-- ADR-098 Addendum 3: de AI-summary creditformule is vereenvoudigd van basis-3-t/m-30min + staffel naar
-- één regel: 1 credit per 10 min video, ⌈duur/600⌉, min 1 (calculate_summary_cost / summaryCreditCost).
-- Rekenkundig identiek vanaf 30 min; alleen kortere video's werden goedkoper. De marge-weergave in het
-- Operations-paneel + per-user-COR MOET dezelfde formule spiegelen als de backend, anders toont het paneel
-- een hogere omzet dan werkelijk geheven voor sub-30-min-samenvattingen (die nu 1 of 2 credits kosten
-- i.p.v. de oude vaste 3). Alleen de credit-expressie 3+⌈(dur-1800)/600⌉ → greatest(1, ⌈dur/600⌉) wijzigt.
-- Toegepast via de Supabase MCP op 2026-08-27; dit bestand houdt de repo in sync (idempotent).
...
declare
  ...
  v_cheapest_eur_per_credit numeric := 0.02;
...
begin
  ...
  classed as (
    select cost_eur, duration,
      ...
      greatest(1, ceil(coalesce(duration,0)/600.0)) as credits,
      greatest(1, ceil(coalesce(duration,0)/600.0)) * v_cheapest_eur_per_credit as revenue_eur
    from per_gen
  ),
  by_class as (
    select dclass, count(*) as n,
  ...
  net as (
    ...
    from ai_summary_usage_log where is_test = false and generated_at >= v_from
  ),
  fin as (select jsonb_object_agg(coalesce(finish_reason,'null'), c) as j from (
  ...
  mdl as (select jsonb_object_agg(coalesce(model,'null'), c) as j from (
      select model, count(*) c from ai_summary_usage_log
      where is_test = false and generated_at >= v_from group by model) s),
  brk as (select count(*) as breaker_fires from transcription_jobs
    where error_type = 'SummaryCostBreaker' and created_at >= v_from)
  select jsonb_build_object(
    'days', p_days, 'cheapest_eur_per_credit', v_cheapest_eur_per_credit,
...
begin
  ...
  per_user as (
    select user_id, count(*) as n_summaries, sum(cost_eur) as cost_eur,
           sum(greatest(1, ceil(coalesce(duration,0)/600.0)) * 0.02) as revenue_worst_eur
