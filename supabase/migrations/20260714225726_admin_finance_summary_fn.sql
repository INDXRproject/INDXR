-- DEEL C — live periode-RPC voor de Finance-tab. Hergebruikt _geld_scope (measured flows/stocks) +
-- opex_accrual (entered). Voegt toe: bankbrug (charged−fee=settled), cache-savings (ai + captions),
-- deferred-schatting (recent verbruiksmix × deferred credits), honest vat_computed.
-- Entered-OPEX = EXTERNAL-ONLY (bedrijfsbreed, één keer) → nooit dubbeltellen bij de test/intern-toggle.
-- NB: opvolgende migratie (amsterdam_dategrain) corrigeert de dag-grain naar Europe/Amsterdam.
...
BEGIN
  ...
  v_days_win   := GREATEST(1, (p_to::date - p_from::date));
  ...
  v_entered := public.opex_accrual(p_from::date, p_to::date);
  ...
  LOOP
    ...
    IF v_internal = false THEN
      ...
      FROM public.daily_cost_counters WHERE day >= p_from::date AND day < p_to::date;
