-- admin_finance_summary: Radar-fraudescreening als GEMETEN OPEX-regel (driver × tarief, geen schatting).
-- Stripe screent élke poging (successful + declined + blocked) à cfg.radar_eur_per_screen; pogingen vóór
-- cfg.radar_free_until zijn gratis (free trial). Screens = geslaagde charges (credit_transactions) +
-- gescreende mislukte pogingen (payment_attempts.screened). Business-wide kost → in EXTERNAL scope geplaatst
-- (zoals funnel_anon en entered-OPEX): geblokkeerde/mislukte pogingen zijn niet aan een user te koppelen en
-- de toggle mag ze niet dubbeltellen. Reconcileerbaar tegen Stripe's Fees report (Reports → All Fees, 96u).
...
BEGIN
  ...
  FOR v_scope, v_internal IN SELECT s, i FROM (VALUES ('external', false), ('internal', true)) AS t(s, i) LOOP
    ...
    -- Radar-fraudescreening (business-wide → alleen external). Driver = screens; tarief = cfg.radar_eur_per_screen.
    IF v_internal = false THEN
      ...
      -- geslaagde screens = geslaagde charges (één per checkout-sessie); billable = op/na free-trial-einde
      ...
      FROM public.credit_transactions ct
      WHERE ct.type='credit' AND ct.metadata ? 'stripe_session_id' AND ct.created_at >= p_from AND ct.created_at < p_to;
      -- mislukte/geblokkeerde screens (alleen die met een charge → Radar heeft echt gescreend)
    ...
    v_opex_meas := v_good + v_fun_ll + v_fun_an + v_fee + v_radar_fee;
    ...
    blocks := jsonb_set(blocks, ARRAY[v_scope], jsonb_build_object(
      ...
      'measured_opex', jsonb_build_object('goodwill', round(v_good,4), 'funnel_loggedin', round(v_fun_ll,4),
        'funnel_anon', round(v_fun_an,4), 'stripe_fee', round(v_fee,2), 'stripe_fee_by_type', v_fee_by_type,
        'radar_fee', round(v_radar_fee,2), 'radar', v_radar, 'total', round(v_opex_meas,4)),
