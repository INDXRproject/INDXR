-- Task A: the refund ledger row's `reason` was written in Dutch ("Gereserveerd … → verbruikt …
-- → … teruggestort (…/… mislukt)") in an otherwise-English UI. Rewrite it to English so NEW rows
-- are stored in English. The account page renders a friendly label from `kind` + `metadata`
-- (structured datacontract), so this text is the audit label; only the string literal changes —
-- amounts, balance mutation, idempotency and metadata are identical to 20260706205835.
...
BEGIN
    ...
    -- English audit label (was Dutch). UI renders a friendly label from metadata, not this string.
    v_reason := format('Reserved %s → used %s → %s %s (%s/%s failed)',
                       v_reserved, v_consumed, abs(v_refund),
                       CASE WHEN v_refund >= 0 THEN 'refunded' ELSE 'charged' END,
                       v_failed, v_total);
