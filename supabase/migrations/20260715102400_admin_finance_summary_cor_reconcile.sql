-- Point 2/3: COR-regel moet sluiten met zijn breakdown. Exposeer per-method AGAINST-REVENUE (gross×share),
-- purchased_share en consumed_by_type zodat de COR-tabel per method optelt tot cor.against_revenue (de regel
-- erboven) en de granted-levering (gross×(1−share)) zichtbaar in OPEX als goodwill staat. Verder ongewijzigd.
...
DECLARE
  ...
  v_gross numeric; v_opex_meas numeric; v_net numeric;
  v_stor_bytes numeric; v_stor_gb numeric;
  v_share numeric;
  ...
  v_defer_credits numeric; v_recent jsonb; v_recent_cor numeric; v_recent_cons numeric;
  v_avg_cpc numeric; v_est_future_cost numeric;
  v_entered jsonb;
...
BEGIN
  ...
  LOOP
    ...
    v_cor_rev := (rec->>'cor_against_revenue')::numeric;   -- (ai+cap+sum+rag)×share
    v_good    := (rec->>'granted_delivery_cost')::numeric; -- (ai+cap+sum+rag)×(1−share)
    v_fun_ll  := (rec->>'funnel_free_caption_cost')::numeric;
    v_share   := (rec->>'purchased_share')::numeric;

    SELECT COALESCE(sum(fee),0), COALESCE(sum(net),0), COALESCE(sum(paid),0), COALESCE(bool_or(tax>0),false)
      INTO v_fee, v_net_settle, v_charged, v_vat_computed
    FROM (
    ...
    IF v_internal = false THEN
      ...
      FROM public.daily_cost_counters WHERE day >= v_from_d AND day < v_to_d;
    ...
    v_cor_rev := v_cor_rev + v_cor_sto;   -- storage volledig tegen omzet
    ...
    SELECT COALESCE(sum(credits_cost) FILTER (WHERE cache_hit),0),
           COALESCE(sum(credits_cost) FILTER (WHERE NOT cache_hit),0),
           count(*) FILTER (WHERE cache_hit), count(*)
      INTO v_ai_hit_cr, v_ai_miss_cr, v_ai_hit_n, v_ai_n
    FROM public.transcription_jobs tj
    JOIN public.profiles p ON p.id=tj.user_id AND p.is_internal=v_internal
    ...
    SELECT count(*) FILTER (WHERE cache_hit), count(*),
           COALESCE(avg(proxy_bytes) FILTER (WHERE NOT cache_hit AND proxy_bytes>0),0)
      INTO v_cap_hit_n, v_cap_n, v_cap_avg_miss
    FROM public.usage_logs
    WHERE extraction_type='caption' AND success AND is_internal_at_time=v_internal
    ...
    v_recent_cor  := (v_recent#>>'{cor,total}')::numeric;
    v_recent_cons := (v_recent->>'consumed_cr')::numeric;
    ...
    blocks := jsonb_set(blocks, ARRAY[v_scope], jsonb_build_object(
      'cash_in', round(v_cash,2),
      'vat', round(v_vat,2),
      'vat_computed', v_vat_computed,
      'revenue_delivered', round(v_revdel,2),
      'deferred_balance', round(v_defbal,2),
      'credits_sold', (rec->>'purchased_cr')::numeric,
      'credits_consumed', (rec->>'consumed_cr')::numeric,
      'consumed_by_type', rec->'consumed_by_type',
      'purchased_share', v_share,
      'balance_cr', (rec->>'balance_cr')::numeric,
      'cor', jsonb_build_object(
        'ai_transcription', round(v_cor_ai,4), 'caption', round(v_cor_cap,4),
        ...
        'against_revenue_by_method', jsonb_build_object(
          'ai_transcription', round(v_cor_ai * v_share,4),
          'caption',          round(v_cor_cap * v_share,4),
          'ai_summary',       round(v_cor_sum * v_share,4),
          'rag',              round(v_cor_rag * v_share,4),
          'storage',          round(v_cor_sto,4))),
      'gross_profit', round(v_gross,2),
      'gross_margin', CASE WHEN v_revdel>0 THEN round(v_gross/v_revdel,4) ELSE NULL END,
      'measured_opex', jsonb_build_object(
        'goodwill', round(v_good,4), 'funnel_loggedin', round(v_fun_ll,4),
        'funnel_anon', round(v_fun_an,4), 'stripe_fee', round(v_fee,2), 'total', round(v_opex_meas,4)),
      'entered_opex_total', CASE WHEN v_internal=false THEN round(v_entered_total,2) ELSE 0 END,
      'net_profit', round(v_net,2),
      'net_margin', CASE WHEN v_revdel>0 THEN round(v_net/v_revdel,4) ELSE NULL END,
      'bank', jsonb_build_object(
        'charged', round(v_charged,2), 'stripe_fee', round(v_fee,2),
      ...
      'cache_savings', jsonb_build_object(
        'ai_transcription', jsonb_build_object('hit_credits', v_ai_hit_cr, 'total_jobs', v_ai_n,
          'hit_jobs', v_ai_hit_n, 'pct', CASE WHEN v_ai_n>0 THEN round(v_ai_hit_n::numeric/v_ai_n,4) ELSE 0 END,
          'saved_eur', round(v_ai_saved,4)),
        'caption', jsonb_build_object('hit_count', v_cap_hit_n, 'total_count', v_cap_n,
          'pct', CASE WHEN v_cap_n>0 THEN round(v_cap_hit_n::numeric/v_cap_n,4) ELSE 0 END,
          'saved_eur', round(v_cap_saved,4))),
      'deferred', jsonb_build_object(
        'balance', round(v_defbal,2), 'credits', v_defer_credits,
        'est_future_cost', round(v_est_future_cost,2),
        'est_future_gross', round(v_defbal - v_est_future_cost,2),
        'window_days', v_defer_win),
  ...
  RETURN jsonb_build_object(
    ...
    'rates', jsonb_build_object(
      'decodo_eur_per_gb', cfg.decodo_eur_per_gb, 'assemblyai_eur_per_min', cfg.assemblyai_eur_per_min,
      'r2_usd_per_gb_month', cfg.r2_usd_per_gb_month, 'r2_free_gb', cfg.r2_free_gb, 'usd_eur_rate', cfg.usd_eur_rate),
    'entered_opex', v_entered,
    'external', blocks->'external',
    'internal', blocks->'internal'
  );
END;
$function$;
