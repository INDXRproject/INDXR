-- _geld_scope: F1 (per-user COR-attributie, PERIODE-share) + F22 (recognized_fee als COR) + F1b (echte deferred_credits).
-- WAT VERANDERT:
--  * cor_against_revenue / granted_delivery_cost / against_revenue_by_method: niet meer scope_COR × gepoolde-all-time-share,
--    maar Σ_user(user_period_cor_method × user_period_share). Period-share = (rec_to.by_user − rec_from.by_user).
--    Voor "All time" identiek aan vroeger; voor een maand nu pas correct (period-COR × period-share i.p.v. all-time-share).
--  * recognized_fee (period) telt mee in cor_against_revenue (revenue-matched, GEEN share, GEEN goodwill).
--  * deferred_credits / deferred_fee / purchased_fee doorgegeven uit _recognize_asof.
-- ONGEWIJZIGD: de scope-totale COR per methode (v_cor_ai/cap/sum/total) — de 'cor'-breakdown blijft VOLLE kost (optie ii);
--    de measured-methode-COR-som verandert niet (geen regressie op cor.total).
...
DECLARE
  ...
  v_ar_ai numeric := 0; v_ar_cap numeric := 0; v_ar_sum numeric := 0; v_goodwill numeric := 0;
...
BEGIN
  ...
  -- Period purchases (FLOW). BTW/measured uitsluitend uit _sale_vat; gross in settlement-EUR.
  ...
  -- Scope-totale measured COR per methode (VOLLE kost; ongewijzigd → COR-tabel toont dit).
  ...
  -- Informatief: gepoolde all-time share (NIET meer gebruikt voor de COR-attributie).
  v_purchased_share := CASE WHEN v_consumed_to > 0 THEN v_cons_purch_to / v_consumed_to ELSE 0 END;

  -- F1 — per-user PERIODE-attributie. user_period_cor_method × user_period_share, gesommeerd.
  -- share_u = (rec_to.by_user[u].purchased_consumed − rec_from...) / (rec_to.by_user[u].consumed_cr − rec_from...).
  SELECT
    COALESCE(sum(uc.ai  * sh.share),0),
    COALESCE(sum(uc.cap * sh.share),0),
    COALESCE(sum(uc.sm  * sh.share),0),
    COALESCE(sum((uc.ai + uc.cap + uc.sm) * (1 - sh.share)),0)
  INTO v_ar_ai, v_ar_cap, v_ar_sum, v_goodwill
  FROM (
    SELECT gu AS uid, COALESCE(a.c,0) AS ai, COALESCE(c.c,0) AS cap, COALESCE(s.c,0) AS sm
    FROM unnest(users) AS gu
    LEFT JOIN (
      -- COALESCE per term: proxy_bytes is NULL voor jobs zonder egress-capture; anders NULL't de hele COR
      -- (dur + NULL = NULL) en verdwijnt de duur-kost stil uit de per-user som (reconcilieert dan niet met de scope-COR).
    ...
    LEFT JOIN (
      SELECT user_id,
        GREATEST(COALESCE(sum((ai_summary_usage->>'prompt_tokens')::numeric),0) - COALESCE(sum((ai_summary_usage->>'prompt_cache_hit_tokens')::numeric),0),0)/1000.0*cfg.deepseek_eur_per_1k_input_tokens
        + COALESCE(sum((ai_summary_usage->>'prompt_cache_hit_tokens')::numeric),0)/1000.0*cfg.deepseek_eur_per_1k_cache_hit_tokens
        + COALESCE(sum((ai_summary_usage->>'completion_tokens')::numeric),0)/1000.0*cfg.deepseek_eur_per_1k_output_tokens AS c
      FROM public.transcripts
      WHERE ai_summary_usage IS NOT NULL AND created_at >= p_from AND created_at < p_to
      GROUP BY user_id
    ) s ON s.user_id = gu
  ) uc
  CROSS JOIN LATERAL (
  ...
  -- COR tegen omzet = per-user measured against-revenue + recognized_fee (fee is al revenue-matched, GEEN share/goodwill).
  -- Storage-COR wordt in admin_finance_summary toegevoegd (external-only). rag=0.
  v_cor_rev := v_ar_ai + v_ar_cap + v_ar_sum + v_recognized_fee;
  ...
  RETURN jsonb_build_object(
    ...
    'against_revenue_by_method', jsonb_build_object('ai_transcription', round(v_ar_ai,4), 'caption', round(v_ar_cap,4),
      'ai_summary', round(v_ar_sum,4), 'rag', 0, 'payment_fee', round(v_recognized_fee,4)),
