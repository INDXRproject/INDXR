-- F15: surface the measured COR/OPEX drivers so each cost row can be recomputed in the UI.
-- The drivers (audio-seconds, proxy-bytes, tokens, storage-GB, free-caption bytes, granted credits)
-- were already MEASURED inside _geld_scope / admin_finance_summary but never returned. This adds a
-- `drivers` object to each scope block + the DeepSeek token rates to `rates`. No number changes —
-- purely additive keys. Display-only support (driver × rate = amount).

-- _geld_scope: add a `drivers` object built from the already-computed local aggregates.
...
-- admin_finance_summary: capture the anon-funnel bytes, bubble _geld_scope drivers into each scope block
-- (augmented with funnel_anon bytes + goodwill granted-credits), and expose the DeepSeek token rates.
...
BEGIN
  ...
  FOR v_scope, v_internal IN SELECT s, i FROM (VALUES ('external', false), ('internal', true)) AS t(s, i) LOOP
    ...
    blocks := jsonb_set(blocks, ARRAY[v_scope], jsonb_build_object(
      ...
      'storage_bytes', v_stor_bytes,
      'storage_approx', v_storage_approx,
      -- F15: drivers behind each COR/OPEX row (from _geld_scope) + anon-funnel bytes + goodwill granted credits.
