-- F2: _geld_scope reads AI-summary COR from the per-run token log on generated_at (the moment the summary
-- ran), not from transcripts.ai_summary_usage on created_at. Regenerate appends a log row → every run counts.
-- Only the two ai_summary token-source blocks change (scope-total + per-user CTE); everything else is identical.
...
BEGIN
  ...
  -- F2: AI-summary COR from the per-run token log (insert-only), attributed to generated_at (the moment the
  -- summary ran), NOT transcripts.created_at. Regenerate appends a new log row -> every run is counted.
  ...
  FROM (
    ...
    LEFT JOIN (
      -- F2: per-user AI-summary COR from the token log, on generated_at (see scope-total block above).
      SELECT user_id,
