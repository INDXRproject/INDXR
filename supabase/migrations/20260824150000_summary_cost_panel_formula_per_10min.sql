-- ADR-098 Addendum 2: de AI-summary creditformule ging van +1/20 min naar +1/10 min boven 30 min
-- (calculate_summary_cost / summaryCreditCost = 3 + ⌈(dur-1800)/600⌉). De marge-weergave in het
-- Operations-paneel + per-user-COR MOET dezelfde formule spiegelen als de backend, anders toont het
-- paneel een andere marge dan werkelijk geheven. Alleen de credit-expressie /1200→/600 wijzigt.
...
begin
  ...
  classed as (
    select cost_eur, duration,
      ...
      (3 + greatest(0, ceil((coalesce(duration,0)-1800)/600.0))) as credits,
      (3 + greatest(0, ceil((coalesce(duration,0)-1800)/600.0))) * v_cheapest_eur_per_credit as revenue_eur
    from per_gen
  ),
  by_class as (
...
begin
  ...
  per_user as (
    select user_id, count(*) as n_summaries, sum(cost_eur) as cost_eur,
           sum((3 + greatest(0, ceil((coalesce(duration,0)-1800)/600.0))) * 0.02) as revenue_worst_eur
