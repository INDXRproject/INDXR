-- FIX: anon-funnel + entered-accrual + dag-telling moeten op de Europe/Amsterdam-kalenderdag draaien,
-- net als de snapshot (die attribueert op snapshot_date = Amsterdam-dag). Anders wijkt live af van de
-- snapshot op de external-rij (anon-funnel viel op de verkeerde UTC-dag). Sluit-test: snapshot == live.
...
BEGIN
  ...
  -- Amsterdam-kalenderdag-grens (consistent met snapshot_finance_day).
  ...
  v_entered := public.opex_accrual(v_from_d, v_to_d);   -- external-only, één keer, Amsterdam-dag-grain
