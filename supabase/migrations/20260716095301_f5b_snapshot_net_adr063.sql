...
DECLARE
  ...
  v_cor_rev  numeric;                                  -- F5b: cor_against_revenue (usage-share + recognized_fee)
  v_fun_ll   numeric; v_fun_an numeric; v_good numeric; v_fee numeric;
  v_radar_fee numeric; v_scr_sb integer; v_scr_fb integer;   -- F5b: radar OPEX (mirror admin_finance_summary)
...
BEGIN
  ...
  LOOP
    ...
    v_cor_rev := (rec->>'cor_against_revenue')::numeric;   -- F5b: usage-share COR + recognized Stripe-fee
    ...
    IF v_internal = false THEN
      ...
      -- F5b: Radar screening fee (billable screens × rate) — mirror admin_finance_summary external branch.
    ...
    ELSE
      v_fun_an := 0; v_stor_b := 0; v_cor_sto := 0; v_radar_fee := 0;
    END IF;

    -- Net (measured) per ADR-063 — identical to admin_finance_summary's net minus the live entered-OPEX overlay.
    -- COR against revenue = usage-share COR + recognized Stripe-fee (deferred fee sits in the Deferred card),
    -- plus storage (flat until F3 per-user attribution). OPEX = goodwill + funnels + radar.
    -- Entered-OPEX is NOT frozen here: it stays a live overlay (editable rows must apply retroactively) — the
    -- Trend subtracts it live. Column is deliberately named net_profit_measured (= measured net before entered).
    v_net := v_revdel
             - (v_cor_rev + v_cor_sto)
