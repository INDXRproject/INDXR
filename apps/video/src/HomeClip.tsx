...
// The raw recording (tests/playwright/capture/recordings/core-flow.webm) is copied into public/ so
// staticFile can serve it. Source ≈ 26.36 s; we play it at 1.25× for a tighter homepage tempo.
...
const Caption: React.FC<{ text: string; durationInFrames: number }> = ({ text, durationInFrames }) => {
  ...
  return (
    <AbsoluteFill style={{ justifyContent: 'flex-end', alignItems: 'center', paddingBottom: 44 }}>
      ...
      >
        <span style={{ width: 9, height: 9, borderRadius: 9999, background: T.accent, display: 'inline-block' }} />
        {text}
      </div>
    </AbsoluteFill>
  )
...
export const HomeClip: React.FC = () => {
  return (
    <AbsoluteFill style={{ background: T.bg }}>
      ...
      <AbsoluteFill style={{ transform: 'scale(1.04)', transformOrigin: 'center center' }}>
        <OffthreadVideo src={staticFile('core-flow.webm')} playbackRate={PLAYBACK_RATE} />
      </AbsoluteFill>

      {CAPTIONS.map((c, i) => (
        <Sequence key={i} from={c.from} durationInFrames={c.durationInFrames}>
          <Caption text={c.text} durationInFrames={c.durationInFrames} />
        </Sequence>
      ))}
    </AbsoluteFill>
  )
