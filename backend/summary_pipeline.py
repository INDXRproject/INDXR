...
def structure_system_prompt(min_sections: int, max_sections: int, total_seconds: int) -> str:
    return (
        ...
        "Produce:\n"
        "1. `overview`: a few short paragraphs summarising the whole video. HARD LIMIT: the overview "
        "must NEVER be longer than the transcript itself — never more words than the transcript "
        "contains. For a short video this is the binding cap and the overview must shrink to fit it; "
        "for a long video it changes nothing, because a genuine high-level overview already sits far "
        "under it. Keep it a high-level summary, not a retelling — the section notes carry the detail.\n"
...
SECTION_SYSTEM_PROMPT = (
    ...
    "HARD LIMIT: your notes must NEVER be longer than the transcript fragment itself — never write more "
    "words than the fragment contains. For a short fragment this is the binding cap; for a long fragment "
    "it changes nothing, because thorough notes already sit well under it. As a rough guide within that "
    "limit, aim for roughly one third of the fragment's word count — a direction, not a floor: write "
    "SHORTER when the fragment carries little real content (small talk, silence, repetition), and let "
    "length follow the information density of the fragment, never the clock. Do not invent, extrapolate, "
    "or pad to hit a length.\n\n"
