...
---
[2026-07-17 10:56] commit: fix(finance): single-row Trend point + drop redundant period toggle

Point 4: remove the Week/Month/Quarter/Year toggle row — it duplicated the
presets (This week == Week, etc.). Presets set the unit; the ← → arrows now
scroll within that unit (This month + ← = previous month), label shows which.

Point 3: the Trend hid its chart entirely below 2 snapshots (text only). A
single day now renders as one centered point; the series/line fills in from the
second nightly snapshot. Zero snapshots still shows the explanatory text.

Docs: LESSONS (env-var-per-deploy-target; verify external API auth+body against
docs before shipping — the F17 Decodo call is Bearer-prefixed + missing
proxyType + wrong date format, root-caused with a proven 200 request, reported
for approval not yet applied). known-issues + priorities updated.

Investigation (reported, not fixed): fetch_service_metrics DOES run (ARQ cron
healthy, worker on latest commit) — proven by service_metrics.last_attempt_at at
02:00 UTC. DeepSeek failed only because its key was absent on the worker (now
set; verified balance $9.99). Decodo fails 401/400 — see known-issues.
...
Changed: docs/LESSONS.md
---
[2026-07-17 15:46] commit: docs(architecture): nightly-jobs.md — factual cron reference + measured Decodo delay

Read-only diagnosis (no code changes). New architecture/nightly-jobs.md documents
the two 02:00-UTC crons as a factual reference:
- snapshot_finance_day (pg_cron): writes yesterday (Amsterdam), idempotent upsert,
  a missed night is a permanent gap (manual backfill only).
- fetch_service_metrics (ARQ worker): sliding 3-day Decodo window, coverage grows
  one day/night, revisions picked up within ~3 days then frozen.

B — measured Decodo registration delay via a real proxy download (Big Buck Bunny,
30.76 MB): registration is near real-time (full +32.2 MB visible 1s after the
download completed), NO revision over 3h (32,670,942 constant at +0/+30/+60/+120/
+180 min), Decodo counts ~4.7% more than us (wire/TLS overhead + handshake reqs).
...
[2026-07-22 14:30] taak: 8 export-format/limits docs-pagina's + 2 schema-fixes + llms.txt verwijderd
- DEEL 1: llms.txt (public/, apps/app/public/, apps/marketing/public/) verwijderd + alle
  verwijzingen; ADR-039 bijgewerkt; writing-standard §A5/§C/§D verwerkt (FAQPage rich result
  gedeprecieerd 7-mei-2026, answer-first blijft, geen inline FAQ in docs).
- DEEL 2: FAQPage acceptedAnswer.text kreeg ReactNode → "" bug — nu reactNodeToText() (platte
  tekst = zichtbare tekst), FAQPage alleen bij faqs.length>0. 18 artikelen kregen BreadcrumbList
  via articlesBreadcrumb(title) (leaf zonder URL). Toegepast op alle 3 templates.
- DEEL 3: export-formats hub + {txt,markdown,csv,srt,vtt,json} + limits geschreven; output-
  voorbeelden uit de échte serializer (packages/shared/src/utils/formatTranscript.ts); getallen
  uit @indxr/shared/lib/{pricing,models}; DocsShell dichtheid max-w-3xl→max-w-2xl (~70ch).
- Build groen; live-verificatie na deploy.

Changed: apps/marketing/src/app/docs/how-indxr-works/export-formats/{page,txt,markdown,csv,srt,vtt,json}/page.tsx,
limits/page.tsx, components/content/templates/{Article,ToolPage,Tutorial}Template.tsx,
components/docs/DocsShell.tsx, lib/{reactNodeToText,breadcrumbSchema}.ts, 3× public/llms.txt (del),
docs/wiki/{decisions/039,content/writing-standard,architecture/sitemap,content/docs-page-contract,
strategy/principles,content/product-truth,architecture/page-structures/docs-vs-articles-density}
---
[2026-07-22 18:19] commit: feat(docs): export-format + limits reference pages, FAQ/breadcrumb schema fixes, remove llms.txt

DEEL 1 — llms.txt removed (public/, apps/app/public/, apps/marketing/public/)
plus every reference; ADR-039 records the verification rationale. writing-standard
§A5/§C/§D processed: FAQPage rich result deprecated (7-May-2026) so FAQPage stays
only where content is genuinely Q&A; answer-first and "no inline FAQ in docs" kept.

DEEL 2 — two JSON-LD schema bugs on the 18 articles:
- FAQPage acceptedAnswer.text received a ReactNode and serialized to "". Now
  reactNodeToText() flattens it to the visible text; FAQPage emitted only when
  faqs.length > 0.
- Articles lacked BreadcrumbList. Added via articlesBreadcrumb(title) (leaf omits
  its URL). Both applied across Article/ToolPage/Tutorial templates.

DEEL 3 — 8 reference pages: export-formats hub + {txt,markdown,csv,srt,vtt,json}
+ limits. Output examples generated by the real serializer
(packages/shared/src/utils/formatTranscript.ts); numbers single-sourced from
@indxr/shared/lib/{pricing,models}; DocsShell density max-w-3xl -> max-w-2xl (~70ch).
...
apps/marketing/src/components/content/templates/TutorialTemplate.tsx
apps/marketing/src/components/docs/DocsShell.tsx
apps/marketing/src/lib/breadcrumbSchema.ts
apps/marketing/src/lib/reactNodeToText.ts
docs/LOG.md
docs/wiki/architecture/page-structures/docs-vs-articles-density.md
docs/wiki/architecture/sitemap.md
docs/wiki/content/docs-page-contract.md
...
op has_captions/0 en toonde het scherm na een cosmetische delay. Hernoemd naar wat het
feitelijk is: de laatste keuze-/kostenstap vóór de start.

- Microcopy: "Check Availability" -> "Review extraction", "Availability Breakdown" ->
  "Before you start", "Free" -> "Using free auto-captions".
- Rode Unavailable-teller nu gevuld met het echte getal uit de playlist-fetch
  (playlist.unavailable_count, yt-dlp: private/members-only/deleted) via nieuwe prop
  unavailableCount, i.p.v. de altijd-0 uit de niet-controlerende stap.
- Nieuw Info-vak: niets wordt vooraf gecheckt; een video zonder captions/onbereikbaar
  wordt tijdens extractie overgeslagen en de gereserveerde credits komen terug.
- Dode route check-playlist-availability (÷8-schatting, geen source-aanroeper)
  verwijderd uit beide apps.
- Docs playlists-pagina + SourcesBlock herschreven naar de echte code.

Reservering/afrekening/teruggave onaangeraakt. pnpm build 2/2 groen.

Co-Authored-By: Claude Opus 4.8 (1M context) <noreply@anthropic.com>
Changed: apps/app/src/app/api/check-playlist-availability/route.ts
apps/marketing/src/app/api/check-playlist-availability/route.ts
apps/marketing/src/app/docs/guides/playlists/page.tsx
docs/LOG.md
docs/wiki/INDEX.md
