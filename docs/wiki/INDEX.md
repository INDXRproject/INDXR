...
| [037-no-comparison-pages.md](decisions/037-no-comparison-pages.md) | Geen comparison pages pre-launch — `/alternative/*` wordt verwijderd *(gedeeltelijk vervangen door ADR-105: comparison pages mogen wél onder `/alternatives/`)* |
...
| [089-remotion-workspace-outside-build-graph.md](decisions/089-remotion-workspace-outside-build-graph.md) | `apps/video` (Remotion) staat buiten de Turborepo-build-graph via pnpm `!`-exclusie — eigen node_modules/install, raakt app-builds/deploys nooit. **Achterhaald voor de demo door ADR-100** (Remotion-compositie verwijderd); `apps/video` blijft enkel voor `export-demos/` |
| [101-optimise-on-activation-not-purchase.md](decisions/101-optimise-on-activation-not-purchase.md) | **Google Ads-campagne stuurt op activatie, niet op aankoop.** Twee redenen: te weinig aankoopvolume voor Smart Bidding (~30 conv/30d nodig, freemium levert veel minder) en time-to-paid 90–180d valt buiten het attributievenster. Activatie = eerste voltooide premium-actie (AI-transcriptie klaar / AI-samenvatting / playlist-video voorbij de gratis drie), NIET signup/caption/pageview/gefaalde job. Nieuw event `premium_action_completed` (server-side) + gezaghebbende kolom `profiles.first_premium_action_at` (atomisch 1×/account via `mark_first_premium_action`). Admin Growth: cost-per-activation + activatie→aankoop-weekcohort + welkomstcredit burn-and-ghost misbruikmetriek. Welkomstcredits 25→50 (activeren vóór het bevestigingsscherm). `credits_purchased`-dubbeltelling weg; `whisper_completed` draagt nu `playlist_id`. |
| [106-summary-breaker-measures-outcome.md](decisions/106-summary-breaker-measures-outcome.md) | **Summary-onderbreker meet uitkomst, niet herstelpogingen + stap-1 truncatie-hardening** (herziet ADR-098 conditie 2). 4/6 demo-summaries faalden: `SummaryCostBreaker` vuurde op `recovery_share > 50%` terwijl `unresolved=0` (samenvatting compleet) — op een korte video (3 secties) tript die cap al bij 2 herstelde secties. Oorzaak: Gemini 2.5 Flash trunceert nu intermitterend (#202) → het stap-2-vangnet (retry→fallback) herstelt vaak; de ADR-098-aanname "herstel=0%" is achterhaald. **Fix A:** `recovery_share` is geen abort meer (alleen health-metriek); breaker meet alleen nog uitkomst (`unresolved>0` / kost/min>€0,02 / abs>€1,50 — kosten zaten al in kost/min, bewezen €0,0008–0,0043). **Fix B:** stap 1 (`_run_structure`, JSON) kreeg hetzelfde retry→fallback-vangnet op een afgekapte 200 (`JSONDecodeError`) i.p.v. de job te laten sterven. Verificatie: 5/5 in één poging (was 2/6), refund-tak ongemoeid (`test_summary_breaker` 7/7). |
| [105-comparison-pages-under-alternatives.md](decisions/105-comparison-pages-under-alternatives.md) | **Comparison pages toegestaan onder `/alternatives/`** (herziet ADR-037 gedeeltelijk). Eigen commerciële namespace, NIET onder `/articles/` (dat blijft redactioneel; vergelijkings*artikelen* blijven verboden). Aanleiding: "otter alternative" ~390 zoekopdrachten/maand US +23% j-o-j + de Ads-campagne heeft een bestemming nodig die de concurrent-intentie beantwoordt (landing-page-experience/Ad Rank). Vaste regels: concurrentcijfers **handmatig + zichtbaar "checked on <datum>" + bronlink, GEEN scraper** (breekt stil → verkeerde prijs over genoemde concurrent = het juridische risico), **geen concurrentlogo's** (alleen naam als tekst), INDXR-getallen uit levende code, verplichte eerlijke "waar de concurrent beter is"-sectie. Eerste pagina: `/alternatives/otter-ai`. |
| [104-transcription-jobs-rls-leak.md](decisions/104-transcription-jobs-rls-leak.md) | **Cross-user RLS-lek op `transcription_jobs` gedicht.** Een PERMISSIVE `USING true`-policy voor `{public}` (misleidend "Service role can do everything" genoemd) overrulede via OR de eigenaarscheck → anon-key + élke ingelogde user zag/muteerde alle 300 jobs van alle 7 users (bewezen). Fix: `DROP POLICY` (service_role heeft BYPASSRLS, geen policy nodig); alleen `auth.uid()=user_id`-SELECT blijft. Clients lezen alleen (Realtime intact), backend schrijft via service_role. Onderdeel van de bredere advisor-audit → [supabase-security-audit.md](operations/supabase-security-audit.md). |
| [103-posthog-identity-across-oauth.md](decisions/103-posthog-identity-across-oauth.md) | **PostHog-identiteit over de OAuth-/verificatie-grens.** `persistence:'memory'` (cookieless, 5(3)) genereert bij élke harde load een nieuwe distinct_id → de redirect naar Google / de verificatielink orphant de pre-signup-id (één user = 3 personen in de 2026-09-01-export, funnel las nul). **FIX A:** anonieme id via `ph_did` URL-param brugpen → `alias()` ná `identify()` op de terugkeer (niks op het apparaat; UUID-guard + strip; lazy `useRef`, koude-load-veilig). **FIX B:** ná expliciete consent `persistence:'localStorage+cookie'` + `cross_subdomain_cookie` (deelt de id over `indxr.ai↔app.indxr.ai`); default blijft `'memory'`. **FIX C:** echte device-tz als person-property + `profiles.device_timezone` (onboarding) + in `/admin/users`. Bekende beperking: #3130 (persistence-wissel mint nieuwe session_id). Live person-merge-meting handmatig (recept in monitoring.md) |
| [102-transcription-first-homepage-positioning.md](decisions/102-transcription-first-homepage-positioning.md) | **Homepage-positionering transcriptie-first** (weg van SERP-title "YouTube Transcript Extractor"). Reden: YouTube-transcript-kop is gratis-markt (competition-index 3–9) + message-mismatch met de live Ads-campagne (biedt op audio/video-upload-termen; landing page experience → Ad Rank → CPC) + AI-Overview verwart merk met `github.com/bahdotsh/indxr`. Homepage draagt nu het betaalde transcriptieproduct: title/meta/H1/hero transcriptie-first, gratis caption-extractie gedemoveerd naar lagere sectie, Organization + SoftwareApplication JSON-LD (`name: "INDXR.AI"`). `/youtube-*` + `/articles/*` ongemoeid (long-tail, dragen niet de merkpositionering). Vervolg: campagne-final-URLs (`/articles/audio-to-text` + `/video-to-text`) zelfde anker. |
| [100-demo-video-real-recording.md](decisions/100-demo-video-real-recording.md) | **Landings-demovideo = échte Playwright-schermopname, geen Remotion-simulatie.** De gesimuleerde HomeClip-compositie (camera/zoom over stills + nep-cursor met hergebruikte fractie-coördinaten → verkeerd vallende kliks) is vervangen door één doorlopende echte opname (`home-clip-video.spec.ts`, core-flow-infra: `recordVideo`/`installCursor`/klik op live bounding box, beide thema's, **geen zoom**). Merk-intro/outro + trims (laadsplash weg, bibliotheek-laad ≤0,5 s) met ffmpeg om de opname (`home-clip-assemble.md`, gestuurd door `timings.json`-marks). Remotion-compositie/scripts verwijderd; `apps/video` enkel nog `export-demos/`. Verfijnt/achterhaalt ADR-089 voor de demo |
| [099-language-detection-provider-plus-text.md](decisions/099-language-detection-provider-plus-text.md) | **Taaldetectie: provider (audio) + tekst, tekst wint bij onenigheid.** Eén verantwoorde talenset op één plek (`language_detection.py`): lingua's 75 min 18 constructed/low-resource long-tail-talen (Tagalog labelde een Engelse lezing fout) + zelfde tekstbudget (300 segmenten/6000 tekens) in AI- én caption-pad. Reconcile-regel (AI): eens→die waarde, oneens→**tekst wint** (wat de gebruiker leest/exporteert; audio-provider kan misdetecteren — labelde een Engelse lezing 'he'), tekst-niets→provider; bij onenigheid beide bewaard (`transcripts.provider_language`). Terugwerkend: 16 rijen met providerwaarde, 15 eens, **1 oneens** (he→en). Nuanceert het provider-eerst-ontwerp. |
| [094-subtitle-segmentation-netflix.md](decisions/094-subtitle-segmentation-netflix.md) | **SRT/VTT-hersegmentatie volgens Netflix TTSG** — woord-niveau cue-builder met harde grenzen 42 tekens/regel · max 2 regels · max 7 s · min 1 s (strenger dan Netflix 5/6 s); passages over de grens verdeeld over meerdere cues op zins- anders woordgrens (nooit midden in woord), evenredige timing. Leessnelheid: doel 20 CPS (Netflix Engels-volwassen; 17 = kinderen), hard plafond **21 CPS = één boven de limiet** omdat wij woordelijk transcriberen i.p.v. condenseren (forceren van 20 gaf gemeten 247 s drift → keuze, geen tekortkoming; 21 geijkt op één snel Engels transcript). Sprekernaam alleen op eerste cue van een beurt, **budget per formaat verschillend**: SRT `Naam:` in-budget, VTT `<v Naam>` buiten budget → SRT≠VTT (VTT minder cues: 3326 vs 3425, gemeten op JRE-transcript, alle harde grenzen 0-overtredingen). Bracket-bevinding: Netflix `[Naam]` off-screen vs onze `Naam:` geeft geen parse-probleem bij platformen (niet gewijzigd). Verfijnt ADR-091. |
| [098-summary-cost-guardrails.md](decisions/098-summary-cost-guardrails.md) | **Kostenbewaking AI-samenvatting** (volgt op ADR-090). **Paneel** `admin_summary_cost_panel` (30d, `is_test=false`): kost/samenvatting median+p99 per duurklasse, **marge op het goedkoopste pakket** (Power €0,02/cr = worst-case), vangnet-aandeel (retry/fallback), finish_reason+model-verdeling, groen/oranje/rood. **Harde onderbreker per taak** (`SummaryCostBreaker` in `run_summary`): stopt + **volledige refund** bij onopgeloste sectie / herstel-aandeel >50% / kost/min >€0,02 (zelf-schalend, straft geen lange video's) / absolute kost >€1,50; bewezen via `test_summary_breaker.py` (7/7, verlaagde grens → vuurt + 100→100). **Rolling-baseline** `check_summary_cost_baseline` (nachtelijk in worker): kost/min 7d vs dag 8–37, WARNING bij ratio >2,0, min-sample-guard; log-tabel. **`is_test`-kolom**: health-metingen (`summary_health.py`) tellen in totaal-COR maar niet in per-user-marge/paneel. **Per-user COR** `admin_summary_cost_per_user`. **Bug gevonden+gefixt:** `credits_refunded` is INTEGER maar 3 summary-refundpaden gaven `True` → 22P02 liet de `status=error`-update falen (job bleef hangen). Onderbouwing uit echte data: herstel 0%, kost/min €0,0006–€0,0030, duurste légale generatie €0,42; >90 min is op Power al verliesgevend (prijssignaal, geen incident). |
| [097-video-upload-formats-and-content-gate.md](decisions/097-video-upload-formats-and-content-gate.md) | **MOV/FLV/AVI/MKV-uploads + inhoud-poort** (9→13 formaten; **sindsdien 15** — OPUS+AAC toegevoegd, zie ADR-097 addendum 2026-08-31). MOV/FLV staan op AssemblyAI's lijst → **rauw doorgestuurd**; AVI/MKV niet → backend **extraheert zelf de audio** (`compress_audio_if_needed(force=True)`, `-vn` mono Opus) vóór submit, gestuurd door de **gedetecteerde container** (`get_audio_container`/`needs_provider_transcode`), niet de extensie. **Matroska-fix:** ffprobe geeft `matroska,webm` voor zowel .mkv als .webm → extensie-hint splitst ze (.webm=raw, anders mkv=transcode; oude `matroska→mkv`-tak was onbereikbaar). **Inhoud-poort:** `has_usable_audio` (ffprobe audio-stream) draait vóór reservering → geen audiotrack = 422 `no_audio`, geen reserve/refund; bewust ruim (extensie hoeft niet met container te matchen). **Browser-fix:** onbekende duur (AVI/MKV) toont geen misleidend grootte-getal meer. Size-tak `compress_audio_if_needed` bleek no-op op upload-pad (drempel=500MB=cap). 500MB-cap ongewijzigd (streamen-refactor → priorities). Verificatie: echte AssemblyAI-runs alle 4 formaten (klein+54,5MB), inhoud-poort 4 gevallen, 7 bestaande formaten regressie, 19 tests groen. |
| [096-measurement-layer-expansion.md](decisions/096-measurement-layer-expansion.md) | **Meetlaag-uitbreiding** (volgt op ADR-092): **snelheid** (fasetijden compress/transcribe/save_ms + RTF=processing/audio → Operations-panel p50/p90/p95/p99), **kwaliteit** (transcript_confidence + language_confidence uit AssemblyAI — antwoord geïnspecteerd: language_confidence alleen in json_response; confidence-trend-panel per taal/week), **bewerkingsgraad** (`transcripts.edit_ratio`, grove woord-multiset-symdiff bij edit-save), **gebruik** (`export_events`-tabel + `trackExport` op alle 9 formaten, RLS insert-own), **kosten** (`transcription_jobs.cost_eur` via `job_cor_eur`/`compute_and_store_job_cost`, single-source rates, cache_hit=0 — marge-query, GEEN COR-bron). RPC `admin_pipeline_metrics` voedt 2 nieuwe Operations-panelen. Grenzen: geen ruwe audio/gedrag/inhoud. Bevinding: RTF p50 0,054/p90 0,124/p95 0,165/p99 0,269 → ~1:10-claim klopt voor mediaan, breekt vanaf p90. |
| [095-download-early-speed-screening.md](decisions/095-download-early-speed-screening.md) | **Vroege snelheidsscreening bij YouTube-audio-download** (volgt op meetlaag ADR-092). Progressie-afhankelijke doorvoervloer **`v_floor(p) = v_norm·(1−p)/(1+p)`** — afgeleid (niet gekozen) uit: geen hervatten → restart-kost ∝ reeds gedownload; forward-kosten (egress + tijd @ `v_norm·c`) van doorgaan vs opnieuw. Hoog bij p≈0 (afbreken gratis), →0 bij p→1 (nooit laat afbreken); `c` (Decodo €/GB) valt weg = schaal-invariant. `v_norm`=287500 B/s (2,3 Mbit/s mediaan) in `cost_config.download_normal_bytes_per_sec`. Meet vanaf eerste byte, ruis-drempel 10s-sample. Screening **retryt met verse sessie, faalt nooit** zolang pogingen over; **laatste poging screent niet**; anti-rondpompen via max 2 aborts. Log `outcome=screen_abort` (doorvoer + moment). Unit-tests bewijzen het mechanisme; 3 echte runs: screening **niet gevuurd** (korte bestanden, transfer <10s — eerlijk gerapporteerd). **Hedging** overwogen, NIET gebouwd (kan egress verdubbelen; beslissen na frequentiemeting). |
| [092-download-throughput-measurement.md](decisions/092-download-throughput-measurement.md) | **Meetlaag YouTube-audio-download-doorvoer** (meten, nog niet ingrijpen — geen doorvoerondergrens/vroege rotatie/formaatwijziging). Per-poging gestructureerde log (`[YT-DLP-AUDIO-ATTEMPT]` video/attempt/**session-id (wachtwoord gemaskeerd)**/bytes/duration_ms/throughput/outcome) + per-job `[YT-DLP-AUDIO-SUMMARY]`. Twee nieuwe kolommen `transcription_jobs.download_ms` + `download_attempts` (via `summary_cb`, forward-only) → doorvoer (`proxy_bytes/download_ms`) en her-download-versterking (`proxy_bytes/download_total_bytes`) direct queryebaar i.p.v. tijdstempel-benadering. Rapportquery voor jobs>1-poging + extra egress (de nu-zichtbare kostenpost). Geverifieerd op echte ondertitelloze video (0,0047 MB/s, 192s voor 0,9MB — ving het trage-exit-IP-patroon). |
| [091-speaker-diarization.md](decisions/091-speaker-diarization.md) | **Sprekerherkenning (diarisatie)** in AI-transcriptie: `speaker_labels=True` (model bepaalt zelf #sprekers), per-segment `speaker` in `transcript.transcript`, sprekerwissel breekt het segment. Hernoemtoewijzing APART in nieuwe kolom `transcripts.speaker_names` (overlay bij tonen+export, origineel herstelbaar, één keer = overal). Kosten: async add-on **+$0,02/u** (NIET in basistarief), `cost_config.assemblyai_diarization_usd_per_hour` + helper `assemblyai_diarization_eur_per_min()` + per-job vlag `transcription_jobs.diarization`, opgeteld bij de twee STT-COR-expressies in `_geld_scope` (creditprijs ongewijzigd; migratie no-op op historie). Export per conventie: TXT/MD `Naam:`-prefix, CSV `speaker`-kolom (alleen bij sprekers), SRT `Naam:`-prefix, VTT `<v Naam>`, JSON/RAG `speaker(s)`. Echte 2-spreker-run geverifieerd (€0,00888 incl. €0,00077 add-on). |
| [090-ai-summary-two-step-structured.md](decisions/090-ai-summary-two-step-structured.md) | **AI-samenvatting herontwerp**: twee modelstappen (structuur + per-sectie uitwerking, beide `gemini-2.5-flash` sinds kostenmeting/addendum 2 — sonnet als fallback; dekking + informatiedichtheid-richtlijn i.p.v. lengte-op-de-klok) + assemblage in code; **splitsing** van een hoofdstuk >2× de mediane duur in delen <mediaan vóór stap 2 (verdunning door gevouwen gat/onder-segmentatie); gestructureerd JSON-schema (overview+sections met klikbare tijdstempels → speler-seek); gateway-`fallbacks`/`fallback_config` i.p.v. handmatige fallback. **Achtergrondtaak** (ARQ `run_summary_job` + poll) op de **gedeelde `transcription_jobs`-tabel met `source_kind='ai_summary'` als discriminator** (erft reserve/refund + watchdog gratis; elke lezer gefilterd — `_geld_scope`/admin-ops/concurrency/watchdog/indicator). **Credit** 3 t/m 30min +1/20min (addendum 2, was /30 en vast 3); reserve==settle, volledige refund bij falen. Per-model COR. Oude summaries hard gewist. Bekende knop: >helft output-tokens is afgerekend denkwerk (`completion_tokens_details.reasoning_tokens`). |
| [088-youtube-ui-in-marketing.md](decisions/088-youtube-ui-in-marketing.md) | YouTube's interface niet in marketingmateriaal tonen (brand-rules vereisen goedkeuring) — clip start bij ons eigen invoerveld; nominatieve tekst blijft toegestaan |
| [087-google-ads-measurement-and-consent.md](decisions/087-google-ads-measurement-and-consent.md) | **Google Ads-meetlaag + consentmodel** (Consent Mode v2 BASIC): eigen consent-laag in `packages/shared`, geo-gesplitst (EEA denied+banner / ROW granted+opt-out) via `x-vercel-ip-country`; niets naar Google vóór toestemming (headless geverifieerd: 0 requests/0 `_gcl_*`); `localStorage` + strikt-noodzakelijk `.indxr.ai`-synccookie `indxr_consent` (alleen de keuze, geen id); `cookie_domain:'auto'` → `_gcl_au` cross-subdomein; purchase- + signup-conversies; **geen GA4** (supersedet ADR-023), Basic i.p.v. Advanced, geen enhanced conversions, eigen banner (TCF = publishers, niet adverteerders). Ads-omzet ≠ Finance-omzet (bruto incl. BTW + Adaptive-Pricing-lijstprijs). /privacy + `LEGAL_VERSION` → 2026-08-02 (bundel, ADR-069). |
...
| [ai-summary.md](architecture/ai-summary.md) | **AI-samenvatting — hoe het NU werkt** (één pagina i.p.v. 5 ADR's): twee modelstappen, waarom stap 2 geen schema gebruikt, het vangnet, de onderbreker, de creditformule (+1/10 min) met onderbouwing, bekende beperkingen. Beslisgeschiedenis blijft in ADR-090/098 |
...
| [google-ads-campagne.md](business/google-ads-campagne.md) | **Google Ads-campagne (gepubliceerd 2026-08-31):** meetinstrument (geen groeikanaal) op het bestandsformaat-cluster (geen gratis uitweg); 4 advertentiegroepen (Voice Memos / M4A-WAV / Open formats / Video containers), 31 keywords, 51 negatieven; afgewezen clusters mét gemeten cijfers; **Maximize clicks met campagnebrede max-CPC €1,80** (bods per groep niet mogelijk) → Max Conversion Value bij 30+ activaties; vooraf vastgelegde evaluatiecriteria (harder bieden = géén optie); open punt: activatieconversie nog niet met echt verkeer geverifieerd. Uitwerking van [ADR-101](decisions/101-optimise-on-activation-not-purchase.md). |
...
| [screenshot-machine.md](content/screenshot-machine.md) | **Playwright-beeldmachine voor docs-screenshots** (`tests/playwright/capture/`): hoe je 'm draait (NODE_PATH-quirk, tegen live app), de 43 assets in `apps/marketing/public/docs/screenshots/`, welke live vs gestubd zijn, dat een hernoemde UI-string 'm laat omvallen (= routecheck), en de ErrorCard kaart↔code-dekkingsanalyse. |
