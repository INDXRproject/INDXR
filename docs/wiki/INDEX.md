...
| [090-ai-summary-two-step-structured.md](decisions/090-ai-summary-two-step-structured.md) | **AI-samenvatting herontwerp**: twee modelstappen (structuur `claude-sonnet-4-6` → per-sectie uitwerking `gemini-2.5-flash`, dekking + informatiedichtheid-richtlijn i.p.v. lengte-op-de-klok) + assemblage in code; gestructureerd JSON-schema (overview+sections met klikbare tijdstempels → speler-seek); gateway-`fallbacks`/`fallback_config` i.p.v. handmatige fallback. **Achtergrondtaak** (ARQ `run_summary_job` + poll) op de **gedeelde `transcription_jobs`-tabel met `source_kind='ai_summary'` als discriminator** (erft reserve/refund + watchdog gratis; elke lezer gefilterd — `_geld_scope`/admin-ops/concurrency/watchdog/indicator). **Credit** 3 t/m 30min +1/30min (was vast 3); reserve==settle, volledige refund bij falen. Per-model COR (sonnet-tarief). Oude summaries hard gewist. |
