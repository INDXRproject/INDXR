...
|---|---|---|
| **Stap 1 — `youtube-transcript-api`** (`extract_via_youtube_transcript_api`) | **JA** — draait via de Decodo-proxy (`GenericProxyConfig`) | **NEE** — geeft geen `proxy_bytes` → geen bump (rest-gap) |
...
- De aanname "`youtube-transcript-api` = geen proxied download / nul Decodo-kost" is **onjuist**: stap 1 is geproxyd en kost dus Decodo-egress — het wordt alleen niet gemeten.
- Gevolg: `daily_cost_counters` **onderschat** de gratis-caption Decodo-kost — het mist álle stap-1-captions (de éérste cascade-stap, slaagt voor veel video's met een ASR- of native manueel spoor). Alleen stap-2/3-captions (yt-dlp) worden geteld.
- Live-confirm van een bump: een video die via stap 2/3 gaat (bv. een zónder ASR-track, zoals Napoleon) én niet gecached is → verse extractie hoogt de dag-teller op met `proxy_bytes`.
