...
| **Stap 2/3 — yt-dlp VTT** (`extract_with_ytdlp`) | JA — proxied VTT-download | **JA** — `proxy_bytes` → `bump_caption_proxy_bytes` (main.py:432 / worker.py:265) |
...
- **Bewuste asymmetrie (eerlijk vermeld):** stap 1 meet de *volledige* proxied egress (watch-pagina + timedtext ≈ 1–2 MB — dat is wat Decodo werkelijk factureert). Stap 2/3 (yt-dlp) meet alleen de VTT-download (~100 KB); yt-dlp's eigen metadata-pagina-fetch loopt óók via de proxy maar wordt daar niet apart gemeten. Beide routes worden nu geteld; stap 1 vollediger. Het resterende yt-dlp-metadata-deeltje is een kleine, aparte verfijning (niet nu).
