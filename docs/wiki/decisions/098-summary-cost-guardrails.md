...
## Addendum 3 (2026-08-27): formule vereenvoudigd naar één regel — basis-3 vervalt

De basis-3-t/m-30min + staffel is vervangen door **één regel: 1 credit per 10 minuten videoduur, naar boven afgerond, minimum 1** — `⌈duur/600⌉`, min 1. Bron: `backend/credit_manager.py` (`calculate_summary_cost`) + `packages/shared/src/lib/pricing.ts` (`summaryCreditCost`, met `CREDIT_COSTS.AI_SUMMARY_PER_10MIN = 1`; de constanten `AI_SUMMARY`/`AI_SUMMARY_BASE_MINUTES`/`AI_SUMMARY_STEP_MINUTES` zijn verwijderd). Dezelfde `⌈duur/600⌉`-vorm als de RAG-export.

**Reden:** één regel is eerlijker en begrijpelijker dan een basis met een staffel — een gebruiker kan de prijs in één zin narekenen. En **geen enkele bestaande prijs stijgt**: de nieuwe formule is rekenkundig **identiek vanaf 30 minuten** (30/60/120/240 min = 3/6/12/24, ongewijzigd t.o.v. Add.2), en **alleen kortere video's worden goedkoper** (0–10 min: 3→1, 10–20 min: 3→2, 20–30 min: 3→3). Exhaustief geverifieerd over 0–40000 s: `nieuw ≤ oud` overal, nergens een verhoging.

**Onderkant-marge (het nieuwe 1-credit-vloertje).** Een samenvatting van ~5 min draait nog steeds de volle pijplijn (structuurstap + 2–3 hoofdstukcalls — `section_bounds` klemt het hoofdstukaantal op minimaal 2, maximaal 3 onder 30 min). Gemeten kost (health-rapport 2026-08-24, echte pipeline-runs, tarief Gemini Flash EU $0,33/$2,75 per 1M ×0,92): een 1:12-video kostte €0,006–0,007, een 10:01-video €0,0175–0,0203; een ~5-min-samenvatting valt daartussen op **~€0,009–0,013 (mediaan)**, tail tot ~€0,017. Tegenover **1 credit netto op de goedkoopste tier (Power, €0,02/cr → €0,0165 na 21% btw)**: mediaan-marge **+€0,004–0,007 (+23 tot +44%)**; alleen de worst-case tail zakt naar ~break-even (−€0,0003). Op alle andere tiers (Try/Starter/Plus) ruim positief. Dat is **beter dan** het al onder Add.2 geaccepteerde Power-profiel (≤30 min-worst −€0,0015). Conclusie: gezond — geen reden om te stoppen.

**Financieel pad geborgd (sterker dan Add.2):** naast de gedeelde bron (`calculate_summary_cost` = reserve/settle/refund; `summaryCreditCost` = app-weergave) is er nu een **gedeelde cross-language fixture** `test-fixtures/summary_cost.json` die beide zijden vastpint, gecheckt door `backend/test_summary_cost.py` + `packages/shared/src/lib/summaryCost.test.ts`, beide gedraaid door `scripts/check-playlist-invariants.sh` — het script wordt ROOD bij elke divergentie tussen backend en TS. `test_summary_credits.py` bijgewerkt (nieuwe verwachtingen). De marge-RPC's (`admin_summary_cost_panel`, `admin_summary_cost_per_user`) spiegelen de nieuwe `⌈duur/600⌉` mee (migratie `20260827120000`), anders overschat het paneel de omzet van sub-30-min-samenvattingen. Alle prose (kostentabel, credits-pagina, samenvattingen-gids, twee artikelen, wiki) rendert uit de bron of is herschreven naar de één-regel-formulering.
