...
**Creditregel** (vervangt de vaste 3): **3 credits t/m 30 minuten videoduur, daarna +1 per begonnen 20 minuten** (`calculate_summary_cost` — bijgesteld van /30 naar /20 in addendum 2 op basis van de marge-meting). *(Achterhaald: de staffel is later /10 geworden (ADR-098 Add.2) en daarna vereenvoudigd naar één regel **1 credit per 10 min video**, `⌈duur/600⌉` min 1 — ADR-098 Add.3.)* Deterministisch uit de duur → reservering == afrekening. Reservering bij job-start, afrekening (settlement gestempeld `product_type='ai_summary'`) bij succes, **volledige teruggave** bij elk faalpad of worker-dood.
