...
4. **`lookup_key` niet hernoemd:** `plus_1300`/`power_3100` behouden het oude creditaantal in de keynaam als bewuste, gedocumenteerde inconsistentie (zie Consequenties).
...
- **Stripe-side (Khidr):** productprijzen → €5/€15/€25/€60 en price-metadata `credits` → 100/400/1.000/3.000. Deze taak raakt Stripe niet aan.
- **`lookup_key`-inconsistentie (bekend & veilig):** `stripeLookupKey`/`stripeProductId` worden nergens in de code gelezen (checkout = inline price_data, webhook = metadata). `plus_1300`/`power_3100` dragen nog het oude creditaantal. Eenzijdig hernoemen in `pricing.ts` zou de mirror met de live Stripe-Price desyncen zonder functioneel voordeel; hernoemen kan alléén samen met een Stripe-side `lookup_key`-transfer. Daarom bewust gelaten en gedocumenteerd (LOOKUP_KEY-NOOT in `pricing.ts`).
