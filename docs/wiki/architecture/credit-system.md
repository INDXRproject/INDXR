...
- **Revenue = purchased-only, granted-first, CHRONOLOGISCH** (ADR-061, 2026-07-15): alleen Stripe-aankopen dragen omzet; verbruik van *granted* credits = acquisitiekost (OPEX). Recognitie loopt nu per verbruiksmoment via `_recognize_asof` (event-stream op `created_at`, granted-first, FIFO purchase-lots met elk hun eigen €/credit) — **niet** meer cumulatief `LEAST(purchased, GREATEST(0, consumed−granted))`. De oude cumulatieve vorm had een **retroactieve clawback**: een goodwill-grant ná aankoop+verbruik verlaagde de al-erkende omzet. Chronologisch raakt een grant van vandaag het verleden (en bevroren snapshots) niet. Invariant: `recognized + deferred = purchased_net`. De COR-splitsing (COGS vs goodwill/OPEX) volgt `purchased_share = purchased_consumed / consumed` (as-of).
