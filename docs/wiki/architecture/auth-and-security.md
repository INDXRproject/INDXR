...
> **Security-audit 2026-09-02 — zie [operations/supabase-security-audit.md](../operations/supabase-security-audit.md).** De volledige Supabase-advisorlijst is toen leeggewerkt. Belangrijkste bevindingen: (1) een cross-user RLS-lek op `transcription_jobs` (always-true policy voor `{public}`, [ADR-104](../decisions/104-transcription-jobs-rls-leak.md)); (2) een financiële drift waarbij `settle_credits` na een `CREATE OR REPLACE` (2026-08-07) opnieuw PUBLIC-EXECUTE kreeg en ~26 dagen door élke ingelogde user aanroepbaar was — nu weer service_role-only; (3) admin-RPC's die anon/authenticated admin-financiële data teruggaven — gelockt. Eindstand: 0 anon-executable DEFINER-functies; de resterende 5 authenticated-executable zijn de bedoelde user-facing RPC's (`get_user_credits`, `deduct_credits_atomic`, `claim_welcome_reward`, `submit_support_ticket`, `library_storage_is_full`). **Les:** een policy-naam is géén rolbinding — controleer de `roles`-kolom van `pg_policies`, niet de naam.
