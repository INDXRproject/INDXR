...
### 2.13b-bis Proxy overhead — is_internal (update 2026-07-17)
`proxy_global_bytes` telt sinds 2026-07-17 **`proxy_usage_log WHERE NOT is_internal`** in de external scope; de internal scope telt `WHERE is_internal`. Testverkeer (bv. de delay-meting, `category='delay_test'`) blijft zo uit de echte (external) economie maar wordt wél in internal geboekt. De account-level reconciliatie-`measured` (§2.13c) telt álle rijen — Decodo factureert het account-breed, dus weglaten zou een vals gat geven.

### 2.13c OPEX-rijen: Proxy reconciliation + Historical proxy (Decodo) (F17/2026-07-17)
1. **Naam:** **alleen external scope** (Decodo factureert account-niveau, niet splitsbaar). Internal = `{status:'not_applicable'}`. Twee rijen, gesplitst op `finance_settings.proxy_measured_from` (2026-07-11 = dag waarop proxy-meting begon):
   - **"Proxy reconciliation (Decodo)"** — dagen ≥ boundary (verified): `gap_bytes = billed_verified − measured`; `gap_cost = GREATEST(0, gap)/1e9 × decodo`.
   - **"Historical proxy (Decodo, unverified)"** — dagen vóór boundary: `unverified_cost = Σ billed/1e9 × decodo`. **Billed IS de kost** (er is geen meting om tegen te reconciliëren) → géén gat mogelijk, géén 100%-gat, en méér dan de vroegere €0.
2. **Formule measured (verified):** `Σ transcription_jobs.proxy_bytes` (beide scopes) + caption-egress (`usage_logs`/`daily_cost_counters`) + `Σ proxy_usage_log.bytes`, **beperkt tot de gedekte dagen ≥ boundary** (UTC-dag-membership). Includeert internal test-rijen zodat het account-level matcht met billed.
3. **Bron:** `decodo_daily_usage` (watermark-upsert door `fetch_service_metrics`, historisch gebackfilld tot 2026-04-21). Boundary uit `finance_settings.proxy_measured_from`. Tarief `cost_config.decodo_eur_per_gb`.
4. **Driver:** ✅ zichtbaar. Reconciliation-hint: `billed X GB · measured Y GB · gap Z GB × €/GB · gap over N of M days · Decodo data starts <dag>`. Historical-hint: `X GB billed × €/GB — before we measured proxy (measurement starts <dag>). Decodo's figure IS the cost; no gap possible.` Uit `<scope>.reconciliation` (`billed_bytes`, `measured_bytes`, `gap_bytes`, `gap_cost`, `coverage_days`, `period_days`, `status`, `data_from`, `measured_from`, `unverified_billed_bytes`, `unverified_cost`).
5. **Status:** `ok`/`partial` (verified data), `unverified_only` (hele periode vóór boundary → alleen historical-rij), `unavailable` (nog geen fetch → cel `—`, geen vals gat).
6. **Scope:** alleen external; beide rijen tellen in external net (`measured_opex.proxy_reconciliation` + `measured_opex.proxy_historical`).
7. **Aannames/zwakke plekken:** **`GREATEST(0,gap)`** (verified) — gemeten kan billed overstijgen (gedecomprimeerd vs wire) → negatief "gat" is meetartefact. Bewezen ≥2 vensters (2026-07-17): This month gap +7,05 MB (2,9% positief) · All time idem + unverified €13,46 · May `unverified_only` €0,68. Decodo-horizon = 21 apr (geen data daarvóór, ook al is `business_start_date` 1 jan).

### 2.13d Waarom géén AssemblyAI-saldo/verbruik-regel (F17, ADR-067)
AssemblyAI biedt **geen** balance- of usage-API — verbruik en tegoed zijn alleen zichtbaar in het dashboard ([assemblyai.com/docs](https://www.assemblyai.com/docs), geen `/account`- of `/usage`-endpoint in de API-referentie). Het account is PAYG met **auto-recharge**, dus er is geen prepaid-saldo dat kan leeglopen. Daarom is er bewust **niets gebouwd** voor AssemblyAI-saldi (in tegenstelling tot DeepSeek prepaid, §Operations). Kostbewaking loopt uitsluitend via de gemeten COR-driver (minuten × `assemblyai_eur_per_min`, §2.4).
