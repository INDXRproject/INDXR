...
### 2.13c OPEX-rij: Proxy reconciliation (Decodo) (F17, ADR-067)
1. **Naam:** "Proxy reconciliation (Decodo)" — **alleen external scope** (Decodo factureert account-niveau, niet splitsbaar). Internal krijgt geen rij (`reconciliation.status='not_applicable'`).
2. **Formule:** `gap_bytes = billed_bytes − measured_bytes`; `gap_cost = GREATEST(0, gap_bytes)/1e9 × decodo_eur_per_gb`. `billed_bytes` = `Σ decodo_daily_usage.billed_bytes` in de periode; `measured_bytes` = **alle** gemeten proxy-bytes = `Σ transcription_jobs.proxy_bytes` (beide scopes) + caption-egress (`usage_logs`/`daily_cost_counters`) + `Σ proxy_usage_log.bytes`. Het positieve gat = onherleidbaar restverkeer → externe economie (zelfde klasse als `funnel_anon`).
3. **Bron:** `decodo_daily_usage` (nachtelijk gevuld door `fetch_service_metrics` uit `POST api.decodo.com/api/v2/statistics/traffic`, dashboard-token `DECODO_API_KEY`). Tarief uit `cost_config.decodo_eur_per_gb`.
4. **Driver:** ✅ **zichtbaar (F15-stijl)**: hint toont `billed X GB · measured Y GB · gap Z GB × €/GB` uit `<scope>.reconciliation` (`billed_bytes`, `measured_bytes`, `gap_bytes`, `gap_cost`, `coverage_days`, `period_days`, `status`). Bij deel-dekking staat `x/y days covered` erbij.
5. **Tijdstoewijzing:** flow — `decodo_daily_usage` per dag, gefilterd op de periode.
6. **Scope:** alleen external. Internal = `{status:'not_applicable'}` (geen rij).
7. **Aannames/zwakke plekken:** **toont pas een gat als er écht data is** — `coverage_days=0` (geen `DECODO_API_KEY`/nog geen run) → status `unavailable`, kost-cel `—` (**niet €0, niet een gat-van-100%**), `last_success_at` in de hint. **`GREATEST(0,gap)`:** gemeten kan billed overstijgen (yt-dlp meet gedecomprimeerde payload, Decodo factureert wire-bytes ná compressie) → negatief "gat" is meetartefact, geen krediet. Forward-only, geen backfill.
