...
- **`cost_config`** — runtime EUR-tarief-bron. Kolommen: `id`, `effective_from`, `currency` (CHECK `='EUR'`), `decodo_eur_per_gb`, `assemblyai_eur_per_min`, `assemblyai_llm_usd_per_1m_input_tokens`, `assemblyai_llm_usd_per_1m_output_tokens` (USD/1M — AI-summary via de AssemblyAI EU LLM Gateway, ADR-068; FX via `usd_eur_rate` op query-moment), `fixed_monthly_infra_eur`, `usd_eur_rate`, `notes`, `created_at`. *(De `deepseek_*`-kolommen — rate/cache/peak/low_balance — zijn gedropt in `20260719122000` toen de summary-provider naar de gateway ging.)* RLS aan, **geen policies** (service-role only). Lees "huidig" tarief = laatste `effective_from <= t`. Geseed 2026-07-11 (Decodo €2,99/GB — **bijgewerkt 2026-07-20 → €3,68/GB** = $4,00 × 0,92, PAYG-prijs gestegen van $3,25 naar $4,00/GB; migratie `20260720130000`; AssemblyAI €0,00322/min, USD→EUR @0,92; DeepSeek input-miss €0,000129/1k, cache-hit €0,000002576/1k, output €0,000258/1k — geverifieerd tegen officiële pricing; peak_multiplier 1,0/geen vensters, migratie `20260711214500`).
...
- **`transcription_jobs.proxy_bytes`** (bigint, nullable) — rauwe pre-ffmpeg Decodo-egress voor de YT-AI/whisper-route; gezet meteen na download (`transcription_pipeline.py`). `file_size_bytes` ongemoeid (upload-only, 0 voor YouTube). **Cumulatief over álle download-pogingen** (BLOK C) — met `download_total_bytes` (één volledig bestand) geeft dit de her-download-versterking.
- **`transcription_jobs.download_ms`** (integer, nullable) — **ADR-092 (2026-08-09)** — som van de actieve yt-dlp download-duren over alle pogingen (ms), excl. retry-backoff en ffmpeg. Doorvoer = `proxy_bytes / (download_ms/1000)`. Gevuld via `extract_youtube_audio(summary_cb=…)` → `_write_dl_summary` op élk eindpunt (succes én mislukking). Forward-only (NULL vóór 2026-08-09).
- **`transcription_jobs.download_attempts`** (integer, nullable) — **ADR-092** — aantal gestarte download-pogingen (>1 = her-download). Met `proxy_bytes` vs `download_total_bytes` = her-download-versterking; rapportquery voor jobs>1-poging + extra egress in ADR-092. Forward-only.
- **`transcription_jobs` meetlaag-uitbreiding (ADR-096, migratie `20260809170000`)** — fasetijden `compress_ms` (ffmpeg-transcode), `transcribe_ms` (submit→provider-completed), `save_ms` (persist+finaliseren) — samen met `download_ms` + `processing_time_seconds` de doorlooptijd; RTF = `processing_time_seconds/duration_seconds`. Kwaliteit: `transcript_confidence` (AssemblyAI `transcript.confidence`) + `language_confidence` (uit `json_response`, geen SDK-attr). Kosten: `cost_eur` (numeric) = gedenormaliseerde per-job COR via `job_cor_eur()` (STT+diarisatie+proxy, cache_hit=0) — snapshot voor marge-query, GEEN COR-bron. Alles forward-only.
- **`transcripts.edit_ratio`** (numeric, ADR-096) — grove bewerkingsgraad (woord-multiset-symdiff÷origineel), gezet bij edit-save (`computeEditRatio`, `packages/shared/src/lib/measurement.ts`). Trend per taal.
- **`export_events`** (ADR-096) — `id, user_id, transcript_id, format` (txt|txt-ts|md|md-ts|json|csv|srt|vtt|rag), `source` (viewer|bulk|rag), `created_at`. Elke ingelogde export gelogd via `trackExport` (fire-and-forget). RLS: authenticated insert-own, geen SELECT-policy (admin leest service-role). Indexen op `created_at`/`format`.
- **`summary_cost_baseline_log`** (ADR-098, migratie `20260824140000`) — insert-only log van de nachtelijke rolling-baseline op de AI-summary kost/min (recent 7d vs basislijn dag 8–37). Kolommen: `id` (PK), `checked_at`, `recent_days`, `prior_days`, `recent_eur_per_min`, `prior_eur_per_min`, `ratio`, `threshold`, `breached`, `recent_n`, `prior_n`, `note`. **Service-role only:** enige schrijver = de SECURITY DEFINER RPC `check_summary_cost_baseline` (worker roept 'm nachtelijk aan), enige lezer = de worker via de service-role key. **RLS-fix (migratie `20260901210140`, geverifieerd 2026-09-01):** de tabel is oorspronkelijk zonder RLS aangemaakt (Supabase-advisor `rls_disabled_in_public`, anon+authenticated hadden volle grants → PostgREST-exposure); gedicht met `ENABLE ROW LEVEL SECURITY` + `REVOKE ALL FROM anon, authenticated` + `GRANT ALL TO service_role`, exact het patroon van `proxy_usage_log`/`service_metrics`/`decodo_daily_usage`. Bewezen: anon REST → 401 `permission denied`, authenticated user → 403, service-role insert onder RLS werkt.
- **`job_cor_eur(duration,model,diarization,proxy_bytes)` + `compute_and_store_job_cost(job_id)` + `admin_pipeline_metrics()`** (ADR-096) — alle SECURITY DEFINER, REVOKE anon/authenticated, GRANT service_role. `admin_pipeline_metrics()` levert de twee Operations-panelen (fasetijd/RTF-percentielen + confidence-trend per taal per week).
- **`transcription_jobs.assemblyai_model`** (text, nullable) — effectief `speech_model_used`, gezet op completion. Waarden: `universal-2`, `universal-3-pro`, `universal-3-5-pro` (streepjes!), of `NULL` voor runs van vóór de capture-kolom (pre-2026-07-11). **Drijft de per-model STT-COR** (ADR-070, zie hieronder).
- **`cost_config.download_normal_bytes_per_sec`** (numeric, ADR-095, migratie `20260809150000`) — verwachte doorvoer van een normale Decodo-exit = **287500 B/s** (2,3 Mbit/s mediaan uit de meetlaag). `v_norm` in de vroege-screening-vloer `v_floor(p)=v_norm·(1−p)/(1+p)`; door de pipeline gelezen en aan `extract_youtube_audio(screen_normal_bytes_per_sec=…)` gegeven (fallback-constante bij read-fout). Her te leiden uit `transcription_jobs.download_ms/proxy_bytes` als de dataset groeit. Geen COR/omzet-effect.
- **`cost_config` per-model STT-tarieven (ADR-070, migratie `20260722101609`):** `assemblyai_stt_usd_per_hour_universal2` (0.15), `_universal3pro` (0.21), `_universal35pro` (0.21), `_fallback` (0.21). USD/uur opgeslagen, `usd_eur_rate` bij query (geen EU-premie op STT). De oude `assemblyai_eur_per_min` blijft bestaan maar wordt **niet meer voor COR gebruikt** (alleen nog `rates`-display).
- **`assemblyai_stt_eur_per_min(p_model text) → numeric`** (ADR-070) — STABLE, SECURITY DEFINER, `REVOKE`'d van anon/authenticated, `GRANT` service_role. Mapt effectief model → EUR/min via de `cost_config`-kolommen hierboven (`usd/uur / 60 × usd_eur_rate`); `NULL`/onbekend → `_fallback`. Enige bron voor de model→tarief-mapping; aangeroepen per run in `_geld_scope` (scope-totaal + per-user against-revenue), nooit scope-gemiddeld.
...
- **`finance_settings`** — key/value config (`deferred_window_days` 30/60/90, `deferred_cost_overrides`, **`business_start_date`** = `"2026-01-01"` (F13, migratie `20260716200000`) — de "All time"-ondergrens + datepicker-floor; **`proxy_measured_from`** = `"2026-07-11"` (migratie `20260717150000`) — de dag waarop proxy-METING begon: Decodo-billed dagen ervóór hebben geen meting → billed IS de kost (geen gat), dagen erná reconciliëren). RLS aan, geen policies.
- **`ai_summary_usage_log`** (F2, ADR-064) — insert-only per-run DeepSeek-tokenlog. `id` (PK), `transcript_id`→transcripts, `user_id`→auth.users, `generated_at`, `model`, `prompt_tokens`, `completion_tokens`, `cache_hit_tokens`, `created_at`. RLS aan met SELECT-policy "read own" (`user_id = auth.uid()`); writes service-role, COR-reads via `_geld_scope` (DEFINER). **Gezaghebbende AI-summary-COR-bron** op `generated_at` — `transcripts.ai_summary_usage` is niet meer de COR-bron. Backend appendt per summary-run (`backend/main.py`). Sinds ADR-068 is `model` = `gemini-2.5-flash` (of de Haiku-fallback) en is `cache_hit_tokens` altijd 0 (gateway heeft geen prompt-cache-tier).
- **`proxy_usage_log`** (F18, ADR-066) — insert-only log van Decodo-proxy-egress die **niet** bij een geleverde job/caption hoort. `id` (PK), `occurred_at`, `category` (`playlist_info`|`metadata`|`caption_failed`|`delay_test`), `bytes` (bigint), **`is_internal`** (boolean default false, migratie `20260717...proxy_internal`). RLS aan, **geen policies** (service-role only). Backend schrijft best-effort via `credit_manager.record_proxy_bytes(category, bytes, is_internal=False)`. Voedt de OPEX-regel "Proxy overhead": **external** telt `NOT is_internal`, **internal** telt `is_internal` (testverkeer uit de echte economie). De account-level reconciliatie-`measured` telt álle rijen (Decodo factureert account-breed → geen vals gat). Disjunct van COR (`status='complete'`). Forward-only.
...
- **`decodo_daily_usage`** (F17, ADR-067) — per-dag Decodo gefactureerd verkeer. `day` (PK, date), `rx_bytes`, `tx_bytes`, `billed_bytes` (bigint), `fetched_at`. RLS aan, **geen policies** (service-role only). Ge-upsert door `fetch_service_metrics` (worker) via een **watermark** (MAX(day)−1 lookback → geen permanent gat bij uitval; zie `nightly-jobs.md`). **Historisch gebackfilld tot 2026-04-21** (Decodo's horizon; 88 rijen). Voedt de Finance-reconciliatie, gesplitst op `proxy_measured_from`: dagen ≥ boundary → billed−measured = gat (~wire-overhead); dagen ervóór → billed IS de kost (OPEX-regel "Historical proxy (unverified)", geen gat).
...
- **`_geld_scope(p_internal, p_from, p_to)`** — range-aware (defaults -inf/+inf → byte-identiek aan oud). FLOWS op `[from,to)`, STOCKS/recognitie cumulatief-`<to`. **F15 (2026-07-16):** retourneert extra `drivers`-object (`ai_transcription.audio_seconds/proxy_bytes`, `caption.proxy_bytes`, `ai_summary.input/cache/output_tokens`, `storage.gb/free_gb/days_win/days_month`, `funnel_loggedin.proxy_bytes`) — de al-gemeten volumes achter elke COR-regel, puur additief (geen getal wijzigt). **F18 (2026-07-16, ADR-066):** retourneert `proxy_fail_bytes` = `Σ transcription_jobs.proxy_bytes WHERE status<>'complete'` (per scope) voor de Proxy-overhead-OPEX-regel. **ADR-070 (2026-07-22):** de STT-audio-COR (`cor.ai_transcription`, beide plekken: scope-totaal + per-user `a`-subquery) wordt **per run** berekend via `assemblyai_stt_eur_per_min(assemblyai_model)` i.p.v. `Σ duur × één statisch tarief` — nooit scope-gemiddeld; NULL-legacy → fallback.
- **`snapshot_finance_day(p_day)`** — idempotente dag-snapshot (DST-aware Amsterdam-daggrens). **F18:** kolom `finance_daily_snapshot.opex_proxy_overhead`; forward-only. **2026-07-17:** external `proxy_global` filtert nu `NOT is_internal` (testverkeer uit de bevroren Trend).
- **`snapshot_finance_catchup(p_max default 7)`** (migratie `20260717...snapshot_catchup`) — de pg_cron `finance-daily-snapshot` roept dit nu aan i.p.v. `snapshot_finance_day()` direct. Vult ontbrekende dagen (< 2 scope-rijen) tussen `MIN(snapshot_date)` en gisteren, meest-recente eerst, max `p_max` per run → een gemiste nacht wordt geen permanent Trend-gat. Idempotent; nooit vóór de reeks-start (schone start 16 juli). SECURITY DEFINER, REVOKE PUBLIC/anon/auth.
...
- **`admin_operations_summary()`** — jobs/success-rate/error-types/retries/capacity/playlist-metrics. **De `services.deepseek`-balanswidget is verwijderd (ADR-068)** samen met de DeepSeek-provider; AssemblyAI (de nieuwe summary-provider) heeft geen balance-API, dus er is geen "External services"-kaart meer.
...
### Nieuwe RPC + Finance-weergave (migratie `20260724221925`)
- **`admin_reversals_summary(p_from, p_to)`** — jsonb `{external, internal}`, per scope `refund{count,amount}` + `dispute{count,amount,fee,open_*,won_*,lost_*}` + `money_out` (= refunds + verloren disputes + alle dispute-fees; open disputes = at-risk, niet in money_out). **Bewust een aparte RPC** — de getal-voor-getal geauditeerde `admin_finance_summary` blijft ongemoeid (geen risico voor de 31/0/0-tally). Scope-split via de user achter de reversal: `user_id` direct, of via `stripe_payment_intent_id` → `credit_transactions` → user; onbekend = external. `SECURITY DEFINER`, `REVOKE ... FROM PUBLIC, anon, authenticated` + `GRANT service_role` (LESSONS 2026-07-13). Geverifieerd op synthetische rijen (external verloren dispute + interne refund, scope-resolutie correct).
- **Finance-tab kaart `ReversalsCard`** (`FinanceView.tsx`) — "Refunds & chargebacks" in de zijkolom (scope-aware, periode-gebonden): refunds, chargebacks (lost/open-at-risk/won/fees), "Money out (settled)", en "Revenue after reversals" (= `revenue_delivered − money_out`, informatief). **Capture-first:** nog niet in de P&L-hero verrekend — die follow-up (recognitie-engine) is bewust apart gehouden. Lege staat toont "No refunds or chargebacks in this period" (huidige productiestand).
