...
- **`<lastmod>` = echte contentdatum per pagina**, uit `sitemap-lastmod.ts` (in de repo, niet op buildtijd uit git — Vercel cloont ondiep). Een route zonder entry in die map krijgt **géén** `<lastmod>`; er wordt nooit teruggevallen op de build-datum of `Date.now()`.
  ...
  - **Backfill (2026-09-03):** drie article-routes stonden wél in `sitemap.ts` maar ontbraken in `sitemap-lastmod.ts` → renderden stil zónder `<lastmod>`. Aangevuld met de laatste content-commit van hun eigen page.tsx: `/articles/supported-formats` → 2026-08-31 (3fdb483/e466b72/8055f7e), `/articles/video-to-text` → 2026-08-31 (3fdb483, cross-link-zin in de body), `/articles/youtube-video-summarizer` → 2026-08-27 (0f82ac7, zichtbare prijs-prose herschreven). Build-geverifieerd: alle 42 sitemap-URL's dragen nu een `<lastmod>` (0 zonder). **Let op:** de routecount hierboven (47) is verouderd — live zijn het er **42** (7 marketing + 21 docs + 14 articles); apart te corrigeren (article-lijst in Laag 2B is eveneens verouderd).
