...
**Premium-bypass gecorrigeerd (2026-08-03):** `/api/extract` bepaalde `isPremium` uit `user_credits.total_credits_purchased` — een DODE kolom die nooit wordt geschreven (0 voor álle rijen, geverifieerd), dus de "wie ooit kocht omzeilt de limiter"-bypass vuurde NOOIT. Nu gelezen uit het gezaghebbende signaal: bestaat er een `credit_transactions`-rij met `kind='purchase'` (alleen door de Stripe-webhook gezet). Geverifieerd tegen live data: de enige koper krijgt bypass, gratis accounts niet. **Let op:** zolang `noopLimiter` actief is (Upstash uit) wordt er tóch niemand gelimiteerd; deze fix zorgt dat het gedrag klopt zódra Upstash terugkomt. Docs (`/docs/reference/limits`) tonen nu 3 tiers (anon / gratis-ingelogd 50/u / na-aankoop geen limiet).
...
### ~~RAG JSON: Settings chunk size ✓ feedback onzichtbaar~~ ✅ Opgelost (geverifieerd 2026-07-24)
**Gevonden:** 2026-04-23 (Sessie 2 test) · **Opgelost:** al gefixt — `DeveloperExportsCard.tsx` toont
een groene `Check`-✓ naast de gekozen preset gedurende 2s na opslaan (`savedValue`-state). Geen actie meer nodig.
...
- [x] RAG JSON: Settings chunk size ✓ feedback zichtbaar — groene Check 2s na opslaan (geverifieerd 2026-07-24)
