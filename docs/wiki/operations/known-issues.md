...
- [~] **Custom SMTP provider configureren voor productie email** — Supabase built-in email service heeft een hardcoded rate limit van 2/h die niet via dashboard verhoogbaar is (custom SMTP vereist). Limiet is project-wide, niet IP-based, en geldt ongeacht Supabase plan-tier (Pro/Team verhogen de default sender limiet niet). Impact: signup, email confirmation, password reset en magic links zijn allemaal gelimiteerd. **Gekozen oplossing: Resend** — SMTP-credentials in Supabase Dashboard → Authentication → SMTP Settings (`smtp.resend.com`, user `resend`, pass = Resend API-key, sender op geverifieerd domein). Met custom SMTP gaat de auth-email rate limit automatisch naar 30/h, verder configureerbaar via Authentication → Rate Limits. **Gebrande confirm/reset-templates zijn geleverd** in `docs/email-templates/` (2026-07-20) — Khidr plakt ze in Auth → Emails (CC had geen Management-API-token om ze live te zetten). Zie roadmap 1.30. **Let op:** dit is Resend-**SMTP**, los van de Resend-**API** (`RESEND_API_KEY`/`RESEND_FROM`) die de support/broadcast-mail in `mail.ts` gebruikt.
