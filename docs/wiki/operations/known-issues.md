...
- **Decodo-reconciliatie: cron draait, maar de API-call is fout (F17-bug, 2026-07-17)** — `fetch_service_metrics` (worker-cron, 02:00 UTC) draait wél (bewezen: `service_metrics.last_attempt_at` 02:00 UTC 17-07, DeepSeek-tak levert saldo $9.99 ✓). De Decodo-tak faalt echter met **401/400**. Root cause (met echte calls bewezen tegen de live key): (a) auth moet de **rauwe** key zijn (`Authorization: <key>`) — mijn code stuurt `Bearer <key>` → 401 "Invalid Api key"; (b) body mist verplicht **`proxyType: "residential_proxies"`**; (c) datums moeten **`Y-m-d H:i:s`** zijn, niet `Y-m-d` → 400 "Invalid data provided"; (d) parser leest dag-veld `key` niet (rijen zijn `{"key":"…","rx_bytes":…,"tx_bytes":…}`). Bewezen werkende request → HTTP 200 met `data[].{key,rx_bytes,tx_bytes}` + `metadata.totals`. **Fix in `backend/worker.py` (`fetch_service_metrics` + `_parse_decodo_traffic`) — gerapporteerd, wacht op Khidr's go-ahead.** Tot dan: reconciliatie blijft "unavailable" (geen vals gat), rest van Finance werkt. `DECODO_API_KEY` staat inmiddels op de worker (len 150).
