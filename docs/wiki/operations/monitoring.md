...
- **UPDATE 2026-09-02 (deel 2): PHASE 2 groen bevestigd + reset-bug gefixt.** Een echte prod-export
  ...
  `SIGNED_OUT`. Zie ADR-103.
- **UPDATE 2026-09-02 (deel 3): $rageclick-artefact gefixt; Google-knop-conclusie ingetrokken.** Grond-
  waarheid (PostHog-UI): de `/login`-`$rageclick` waren de **e-mail/wachtwoord-veldlabels** (niet de
  Google-knop) en de tweede was de **"Upload"-label** — beide `<label htmlFor>`-dubbelvuur (2 clicks per
  tap: label + gesynthetiseerde control-click). **Geen UI-probleem, geen pending-state nodig.** Structureel
  gefixt met `ph-no-capture` op het redundante element (shadcn `<Label>`-primitive → alle formulierlabels;
  native support/contact-labels; en de off-screen upload-input). `$rageclick` is nu weer betekenisvol.
  Meet-regel: één tap moet één autocapture-event geven — zie LESSONS 2026-09-02.
...
- **Sentry alerting rules:** Sentry is geconfigureerd maar nog geen alert-regels ingesteld voor watchdog-errors of financiële failures.
  - **Watchdog run-status signaal (`watchdog_interrupted_jobs`):** elke pass vangt zijn eigen query-fout als WARNING en gaat door (één dode pass blokkeert de rest niet). Kon een pass zijn werk NIET doen, dan emit de cron aan het eind één ERROR-event `[WATCHDOG] run DEGRADED …`; bij succes logt hij `run ok`. **Herhaalt het DEGRADED-event elke run (elke 2 min) → structureel; één keer → blip.** Sinds 2026-09-11 draagt het DEGRADED-event ook het exception-type + de message per gefaalde pass: tag `failed_passes` (welke passes, bv. `1a`), tag `failed_pass_types` (queryable exception-typen, bv. `APIError`), en context `watchdog_pass_failures` (per pass `error_type` + `error_message`) — plus alles in de logregel. Zo is uit het event zelf te zien WAAROM een pass viel zonder de losse WARNING erbij te zoeken. **NB — niet elke pass-fout is de HTTP/2-GOAWAY-bug** (die is gefixt, ADR/commit 2638a72, singleton `http2=False`): een Supabase-edge `504 Gateway Timeout` op een enkele pass-query is een aparte, transiënte oorzaak (de volgende run herstelt vanzelf). Verifieer de frequentie via de Railway-worker-logs (`[[reference_railway_worker_logs]]`) vóór je een fix stapelt.
