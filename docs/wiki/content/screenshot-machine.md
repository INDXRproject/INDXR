...
- **Vast venster + thema + sessie**, net als de beeldstandaard: viewport 1280×720, `colorScheme:'light'`, consent gezet, gedeelde `account1`-sessie uit `capture-state.json` (dezelfde `global-setup.ts`), fixture-video `kBdfcR-8hEY`. De opname toont **echte fixture-titel + duur** en de kostenkaart van het **AI-transcriptiepad** (54:56 → 55 credits, `ceil/60`) — dat is de bewuste demo van het betaalde pad, niet de kost van dit transcript zelf (de fixture komt uit gratis captions; een AI-**samenvatting** ervan kost apart 6 credits). Het transcript in beeld zijn de **echte** fixture-captions (verbatim uit `homeExportSamples.ts`).
...
Runtime: de default-set (21 assets) draait in **~1 min** tegen `app.indxr.ai`.
...
**AI-summary seed — BEWUST, ECHTE samenvatting in de PRODUCTIE-DB (BEVROREN 2026-08-25):** de drie `summary-*`-captures (overview / chapter / edit) lezen een **echte**, door het live product gegenereerde AI-samenvatting (ADR-090) op de **Justice-fixture** (transcript `0798fa30-8056-4343-9e02-c50d93c00e4a`, `kBdfcR-8hEY`). Bevroren cijfers: `gemini-2.5-flash` twee-staps, schema v2, **5 hoofdstukken**, 3632 samenvattingswoorden (overview 125 + hoofdstukken 3507), ratio 52%. **Kosten: het transcript kwam uit captions = 0 credits; de samenvatting kost 6 credits** (`calculate_summary_cost(3282)`) — NIET de 1-cr/min AI-transcriptieprijs. De Justice-rij heeft een videoId → de sectie-tijdstempels zijn de **klikbare amber Play-knop** (met videoId) i.p.v. de statische klok. **Deze samenvatting mag NIET worden opgeruimd en NIET opnieuw gegenereerd voor een proef/meting** — laat het transcript én zijn `ai_summary` staan (zoals het "Designing for Deep Work"-interview); zie de LESSONS-regel over fixtures die een schermafbeelding voeden. Moet hij ooit terug, klik "Regenerate summary" op die rij (6 credits); model-uitvoer is niet-deterministisch, dus hoofdstukindeling/tekst kunnen verschillen → dan opnieuw meten + captures opnieuw schieten. Volledige cijfers, de 5 koppen met tijdspanne + het verbatim voorbeeldfragment (overview + hoofdstuk 1): [content/summary-example-justice.md](summary-example-justice.md).

**Summary-bewerkmodus-capture (2026-08-24):** `summary-edit` schiet de bewerkbare samenvatting (`?tab=summary_edited`) op DEZELFDE Justice-fixture: de tab-strip met "Edited summary" actief, de Tiptap-editor met opmaakwerkbalk, en de hoofdstukken **mét tijdstempels** (de seed `summaryToTiptapDoc` zet de tijdstempel vóór elke hoofdstukkop). De editor wordt **geseed uit de gegenereerde samenvatting en NIET opgeslagen** — er blijft dus geen `ai_summary_edited` en geen demo-tekst op de fixture-rij achter (belangrijk: die rij voedt ook de andere captures + het artikelvoorbeeld). Geschoten met `topShot` (frame van de tab-strip t/m de editor).
...
## Assets (42 dual-theme = 21 onderwerpen × licht/donker) — LIVE vs GESTUBD
...
| **LIVE — AI summary (2026-08-24)** | `summary-overview` (de `AiSummaryView`-kaart: "AI Summary"-kop + **Copy · Export · Regenerate · Edit** (Regenerate toegevoegd bij het toolbar-herontwerp 2026-08-25), de overkoepelende samenvatting en het eerste hoofdstuk met amber `▶`-tijdstempel eronder; via `topShot` geclipt tot één scherm), `summary-chapter` (één hoofdstuk — kop, klikbare tijdstempel, uitgewerkte notities), `summary-edit` (de **bewerkmodus**: tab-strip met "Edited summary" actief + Tiptap-editor met opmaakwerkbalk + hoofdstukken mét tijdstempels; geseed uit de gegenereerde samenvatting, NIET opgeslagen — zie bewerkmodus-noot). Alle drie lezen de **echte** gegenereerde samenvatting op de Justice-fixture (`?tab=summary` / `?tab=summary_edited`, zie AI-summary-seed-noot). **`topShot`** (naast `frameShot`): voor React-gerenderde kaarten die te hoog zijn — een op de kaart gezette `maxHeight` wordt door een re-render weggevaagd (i.t.t. de imperatieve Tiptap-`.ProseMirror`), dus `topShot` bouwt hetzelfde gecentreerde frame op de viewport-oorsprong en gebruikt `page.screenshot({clip})` (vaste viewport-regio) — de hoogte klopt gegarandeerd, de onderrand is een schone afsnede |
