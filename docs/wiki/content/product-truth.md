...
- **Duur:** **3296 s**. **Transcript-kosten van deze fixture: 0 credits** — hij is uit **YouTube-captions** geëxtraheerd (zie de caption-regel hierboven; caption-extractie is gratis). De AI-transcriptieprijs zou `ceil(3296/60)` = **55 credits** zijn (pricing.ts, 1 cr/min) — dat getal is de **illustratie van het betaalde AI-pad in de core-flow-videodemo**, niet de werkelijke kost van dit transcript. De AI-**samenvatting** van dezelfde video kost apart **6 credits** (`calculate_summary_cost`, zie hieronder).
...
**Echte AI-samenvatting op deze fixture (bevroren, 2026-08-25).** Eén door het live product gegenereerde AI-samenvatting (ADR-090) van de Justice-fixture is als bevroren bron voor het samenvatten-artikel vastgelegd: duur 54:42, transcript 6987 woorden (uit **captions → 0 credits**), **5 hoofdstukken**, 3632 samenvattingswoorden (overview 125 + hoofdstukken 3507; ratio 0,520 ≈ 1:1,9), **samenvatting-kosten 6 credits** (`calculate_summary_cost(3282)` = ⌈3282/600⌉, ADR-098 Add.3 — 1 credit per 10 min video; ongewijzigd t.o.v. de oude staffel want >30 min is identiek), `gemini-2.5-flash` twee-staps, schema v2. Volledige cijfers, de 5 koppen met tijdspanne én het verbatim voorbeeldfragment (overview + hoofdstuk 1) staan in [content/summary-example-justice.md](summary-example-justice.md); de `summary-overview`/`summary-chapter`/`summary-edit`-captures lezen deze samenvatting — **bevroren fixture, niet opnieuw genereren voor een proef** (zie [screenshot-machine.md](screenshot-machine.md) + de LESSONS-regel).
