...
- [ ] **1.31 — Upload-cap verhogen boven 500 MB vraagt streamen (genoteerd 2026-08-12, bij ADR-097)** — De 500 MB-grens is een **productkeuze**, niet een AssemblyAI-limiet (die is 10 u + 5 GB). Verhogen is **niet** een one-liner: het upload-endpoint leest het **hele bestand in RAM** (`backend/main.py` `audio_content = await audio_file.read()`) vóór het naar temp schrijft → piek-RAM = bestandsgrootte × tot 3 gelijktijdige jobs (`MAX_CONCURRENT_JOBS`), ~1,5 GB bij 500 MB. De uploadroute zelf is géén blocker (browser→Railway direct, bypasst Vercel 4,5 MB) en de achtergrondtaak-timeout is duur-gebonden (10 u), niet MB-gebonden. **Omvang:** eerst de upload-afhandeling herbouwen naar streamen-naar-schijf (`UploadFile` chunked naar temp i.p.v. `.read()`), dán pas de cap veilig verhogen. Beslissing later, op feiten.

- [x] **1.30 — Custom SMTP voor transactionele auth-mail (Resend)** — **✅ AFGEROND + E2E-geverifieerd 2026-07-20 (was launch-blocker)**
  Supabase's ingebouwde auth-mailer (signup-confirm, password-reset) draaide op een gedeelde SMTP met strakke rate-limits (~enkele mails/uur) en matige deliverability — niet productie-waardig. **Opgelost:** Resend-SMTP gekoppeld in Supabase Auth (`smtp.resend.com`, sender `no-reply@send.indxr.ai`), auth-rate-limit op 30/u; gebrande confirm+reset-templates uit `docs/email-templates/` live geplakt in het dashboard. **E2E-bewijs (Khidr, 2026-07-20):** signup-confirm-mail kwam aan via Resend (Resend Logs: `/emails` 200), logo laadt in de inbox, verify-link → onboarding, 25 welcome-credits pas ná verificatie; password-reset-mail kwam aan met correcte recovery-redirect (`/dashboard/settings?reset=true`). Koppelde aan 1.19 (verificatie AAN) en 1.12 (welcome-credits ná verificatie — al vervuld, zie noot hieronder).
  > **Diagnose (2026-07-20):** transactioneel (GoTrue-ingebouwde mailer) vs. broadcast/support (Resend-**API** in `apps/app/src/lib/mail.ts`) is volledig gescheiden — custom SMTP koppelen raakt `mail.ts` niet (Resend-API ≠ Resend-SMTP, zelfde account/domein). Auth-flows in `packages/shared/src/actions/auth-actions.ts` (signup `:140`, reset `:270`, resend `:288`); **geen magic-link** (geen `signInWithOtp`). Auth-config/SMTP is **niet via de Supabase-MCP leesbaar** — status is een dashboard-check.
  > **CODE (CC, gedaan 2026-07-20):** gebrande auth-mail-templates (confirm + reset) in `send.indxr.ai`-huisstijl, versiebeheerd in `docs/email-templates/` (`{{ .ConfirmationURL }}` intact, e-mail-safe tabel-layout). Geen Management-API-token beschikbaar → **niet** live gezet; Khidr plakt ze in het dashboard (zie README daar).
  > **DASHBOARD (Khidr, open):** (1) Resend-SMTP koppelen in Supabase → Auth → SMTP Settings (`smtp.resend.com`, user `resend`, pass = API-key, sender op geverifieerd domein); (2) auth-rate-limit 2/u → 30/u; (3) confirm+reset-templates plakken uit `docs/email-templates/`; (4) E2E inbox-test (kan pas ná SMTP-koppeling). **1.12-noot:** "welcome-credits ná verificatie" is architecturaal **al** vervuld — `claim_welcome_reward` draait bij onboarding-afronding (`auth-actions.ts:243`), onbereikbaar zonder geverifieerde sessie zolang verificatie AAN staat. Geen code nodig.

- [ ] **1.31 — Sentry noise-filtering vóór launch** — **belangrijk (pre-launch)**
  > **Noot (2026-07-19):** dit item = *noise-filtering* (`ignoreErrors` op bekende ruis) en staat nog **open**. De Sentry **PII/replay-scrubbing** (privacy, waar de LIA op leunt) is een aparte zorg en is wél afgerond in de privacy-hardening-taak van 2026-07-19 (`sendDefaultPii:false` + `beforeSend`-scrub op alle configs, Session Replay uit; backend `send_default_pii=False` + `before_send`; backend-PostHog naar EU + geoip uit). Zie `business/privacy-facts.md` + `operations/known-issues.md` (GDPR-sectie). De taakopdracht noemde dit "1.31" maar dat nummer is in deze roadmap de noise-filtering — vandaar geen `[x]` hier.
...
- [x] **1.32 — PostHog privacy-by-design (EU + cookieless + geen recording)** — **gedaan 2026-07-18** (commit privacy-delete-cleanup / posthog-eu-cookieless)
  Opgelost door session-recording **volledig uit** te zetten (`disable_session_recording: true`) i.p.v. veld-masking — geen DOM-opname = geen PII-lek in recordings, sterker dan `maskAllInputs`. In dezelfde fix: `api_host` → EU (`https://eu.i.posthog.com`, via env), `persistence: 'memory'` (cookieless — geverifieerd: 0 cookies/localStorage/sessionStorage in een live browser), e-mail uit `identify()` (alleen pseudoniem `user.id`), en `$ip` genulld via `before_send` (geverifieerd: input-IP → `null`). Server-side posthog-node in de webhook: `disableGeoip: true`. Admin-deeplinks → `eu.posthog.com`. **Khidr-actie:** verse EU-projectkey op Vercel + lokaal zetten (`NEXT_PUBLIC_POSTHOG_KEY`/`_HOST`/`_PROJECT_ID`) en de US-instance opzeggen zodra EU events ontvangt. Zie ADR-023 (privacy-sectie). Koppelt aan 1.18 (privacy-beleid: PostHog nu écht cookieless → cookie-consent voor analytics vervalt).
...
- [x] **F10 — Presets:** This week · This month · Last month · This quarter · Last quarter · This year · All time · Custom. ✅ (2026-07-16, herzien 2026-07-17) `presets()` in `periods.ts`; presets-rij + Custom-datumrange in `PeriodPicker`. **De aparte Week/Month/Quarter/Year-toggle is verwijderd** (dubbel met de presets) — de pijltjes ← → bladeren binnen de eenheid van de actieve preset ("This month" + ← = vorige maand), het label toont welke periode. Kwartaal = OSS-cyclus.
...
- [x] **Checkout-consent-checkbox voor §7 (herroepingsverlies)** — ✅ **GEDAAN 2026-07-20 ([ADR-069](../decisions/069-terms-acceptance-at-checkout.md)).** Verplicht vinkje "I agree to the Terms of Service and Privacy Policy" (beide klikbaar) vóór de betaling, **server-side afgedwongen** (checkout-route `400` zonder `termsAccepted`; ook de marketing-auto-checkout redirect niet meer stil door). Aanvaarding vastgelegd in `terms_acceptances` (user/timestamp/`terms_version`/`stripe_session_id`) + `session.metadata.termsVersion`. Geverifieerd: aankoop geblokkeerd zonder vinkje (400), RLS staat alleen eigen-user-inserts toe, record draagt alle velden. **Restpunt (juridisch, buiten code):** de box incorporeert §7 per referentie (waivertekst staat in de Terms, niet in de box — bewuste keuze). Of art. 16(m) CRD / 6:230p BW een **aparte, expliciete** "directe uitvoering + verlies herroepingsrecht"-bevestiging vereist naast de Terms-acceptatie → laten toetsen door juridisch advies vóór launch.
...
- [ ] **Klik-ID's worden account-lang bewaard, terwijl ze na Google's attributievenster nutteloos zijn (genoteerd 2026-08-30, ADR-101).** `profiles.gclid/gbraid/wbraid` + `click_id_at` staan er zolang het account bestaat (verwijderd bij accountverwijdering). Functioneel is een klik-ID alleen bruikbaar binnen Google's conversievenster (~90 dagen); daarna heeft bewaren geen doel meer. Onder **AVG art. 5(1)(e) (opslagbeperking)** zou een periodieke opschoning netter zijn: een nachtelijke job die `gclid/gbraid/wbraid/click_id_at` op `NULL` zet zodra `click_id_at` > 90 (of 180) dagen oud is. **Niet gebouwd** — opslag-only (ADR-101), geen deletion-cron. Vastgelegd zodat het niet verdwijnt; bouwen of termijn kiezen = beslissing voor Khidr.
...
- [ ] **TEST 6 — Testaankoop (dubbel doel, na ADR-087):** één echte aankoop per tier verifieert (a) de keten **checkout → webhook → `add_credits`** tegen de huidige 14-juli-prijzen (€5→100, €15→400, €25→1.000, €60→3.000 — de oude TEST 4 draaide nog op €3,49/100) én (b) de **Google Ads purchase-conversie** (ADR-087): op de successtate vuurt de conversie met `value`=`eurForCredits(credits)` (bruto incl. BTW) + `transaction_id`=Stripe-session-id, en een refresh telt niet dubbel (localStorage-guard + Google-side dedup). Code-pad is af + het consent/tag-gating is headless bewezen; alleen de echte aankoop ontbreekt (Khidr). Vereist consent op de app-host (of eerder op marketing — synct via `.indxr.ai`-cookie).
...
- [x] **Docs-gat: opslaglimiet (max + handhaving) in Limits** ✅ 2026-08-03. Op `/docs/reference/limits` staat nu een **Library storage**-sectie (uit `@indxr/shared/lib/storage`: basis 100 MB, max 500 MB, +100 MB per 100 credits tot 4×) + prose over de handhaving (transcript geweigerd vóór credit-reservering → géén credits kwijt, bestaande transcripten blijven) + kruislink naar Credits + de `storage_full`-ErrorCard-figuur in de nieuwe beeldstandaard. Eenheid-check gedaan: bibliotheekcap (MiB, product-label "MB") is een ánder getal dan de per-bestand-uploadcap (500 MB decimaal); expliciet als verschillende limieten gepresenteerd. Getallen geverifieerd tegen storage.ts + migraties 20260723140000/20260724013956, niet de wiki.
- [~] **`docs/content/*.md`-mirrors afgebouwd (drift-bron) — grotendeels klaar 2026-08-03.** Uitgevoerd: (1) de 6 **wees-drafts** zonder gerenderde route — `ARTIKEL-alternative-{downsub,happyscribe,notegpt,tactiq,turboscribe}.md` + `ARTIKEL-youtube-transcript-generator.md` — **verwijderd** (goedkeuring Khidr): de vergelijkingsdrafts botsen met [ADR-037](../decisions/037-no-comparison-pages.md) (geen comparison pages) en met het campagnebesluit in [keyword-demand-2026-08.md](../business/keyword-demand-2026-08.md) (niet bieden op concurrentmerken); de generator-draft dekt een intentie die `/transcribe` al bedient. (2) De **echte mirrors waarvan de gerenderde `.tsx` de inhoud volledig dekt** (per-bestand geverifieerd via subagent-vergelijking) — 10 stuks — **verwijderd**: `ARTIKEL-{blog-youtube-channel-knowledge-base, blog-youtube-transcripts-vector-database, bulk-youtube-transcript, youtube-age-restricted-transcript, youtube-members-only-transcript, youtube-playlist-transcript, youtube-srt-download, youtube-transcript-markdown, youtube-transcript-obsidian, youtube-transcript-without-extension}.md`. (3) Twee van de gap-mirrors zijn op **2026-08-03 alsnog verwijderd** (commit `82ea3d5`, "delete two stale content mirrors"), omdat hun gap stale/tegenstrijdig was: **`ARTIKEL-youtube-transcript-csv.md`** (merged/playlist-CSV-sectie die de `.tsx` bewust verving door "ZIP met losse CSV's") en **`PRICING-PAGE.md`** (concurrent-prijstabel Rev/HappyScribe/TurboScribe/Otter — botst met ADR-037 + stale prijzen). **NOG OPEN — 4 mirrors met een echte gap** (bestaan op schijf per `git ls-files docs/content/`, geverifieerd 2026-08-04; de gerenderde pagina dekt niet alles; bewust laten staan tot iemand port-of-dropt): **`ARTIKEL-blog-chunk-youtube-transcripts-for-rag.md`** — nog niet gerenderd: FAQ "chunk at extraction vs export time" (minor); **`ARTIKEL-youtube-transcript-for-rag.md`** — nog niet gerenderd: het LlamaIndex-codevoorbeeld ontbreekt op de `.tsx`; **`ARTIKEL-youtube-transcript-json.md`** — nog niet gerenderd: sectie "Does This Match youtube-transcript-api?" + JSONL-FAQ + merged-JSON-FAQ; **`ARTIKEL-youtube-transcript-not-available.md`** — nog niet gerenderd: FAQ "search binnen transcript geeft geen resultaten" (minor). Besluit per bestand (Khidr): content porten naar de `.tsx` of de gap bewust laten vallen en het bestand alsnog verwijderen. `ARCHITECTURE.md` blijft — echte docs, geen page-mirror. `product-truth.md` bijgewerkt (sync-regel geldt nu alleen nog voor deze 4).
...
- [ ] **Artikel-kruislinks + "Related"-component (etappe D/E, NIET eerder)** — de 18 artikelen linken nu nergens naar elkaar en hebben geen "lees ook"-blok (link-island, writing-standard §B). Bouw het `Related`-component en leg de kruislinks tijdens de artikel-herschrijfronde, niet als losse taak ervoor. **[writing-standard §B/§C4; ADR-074]**
- [ ] **robots.txt — AI-crawlers expliciet toestaan (pre-launch)** — de ophaal-crawlers die citaties opleveren moeten expliciet `Allow`: **OAI-SearchBot, Claude-SearchBot, Claude-User, PerplexityBot, Perplexity-User**. Een geblokkeerde ophaal-crawler = onzichtbaar in die engine. `Google-Extended` en `anthropic-ai` zijn **trainings**-crawlers → vrije keuze (mag blokkeren). Verifieer de huidige `robots.txt`-allowlist en vul de ophaal-crawlers aan. **[writing-standard §B/§C12; ADR-074]**
- [ ] **Khidr-actie vóór livegang: Search Console én Bing Webmaster Tools aansluiten** — Bing is de index waar ChatGPT uit ophaalt; zonder Bing-indexering geen ChatGPT-citaties. Beide property's verifiëren + sitemap indienen. **[writing-standard §B; ADR-074]**
- [!] **llms.txt bestaat NIET (genoteerd 2026-08-27).** Een indexerings-/campagnetaak vroeg om "de bestaande llms.txt bijwerken", maar er is er geen — niet in `apps/marketing/public/`, geen route, en `https://indxr.ai/llms.txt` geeft 404. Het is bewust uitgesteld in [ADR-039](../decisions/039-llms-txt-low-priority.md) (llms-txt-low-priority). Als de campagne er één wil, is dat **nieuw werk** (aanmaken), geen opschoning — beslissing voor Khidr: bouwen of uitgesteld laten. (Niet aangemaakt in deze ronde: buiten de "opschoning, geen herontwerp"-scope.)
- [!] **Artikelindex = 12 levende artikelen, niet 9 (genoteerd 2026-08-27).** Een eindcontrole verwachtte "negen levende artikelen"; de index (`/articles`) toont er **12**, allemaal echte pagina's, correct gecategoriseerd (Troubleshooting 2, Export Formats 2, Workflows 5, Deep Dives 3), en geen enkele verwijderde/hernoemde slug erin. De "9" was waarschijnlijk de oude ~10-set ná de 2026-08-07-samenvoeging min de vandaag verwijderde `youtube-transcript-without-extension`, zónder de recent toegevoegde artikelen mee te tellen. De index is intern consistent (sitemap.xml = 41 routes, elke pagina bestaat, geen stale slug); **geen actie tenzij Khidr bewust een kleinere gecureerde set wil** — dan is dat een contentbeslissing, geen consistentie-fix.
...
- [ ] **Login-providers uitbreiden (genoteerd 2026-08-31, Khidr).** Nu alleen e-mail/wachtwoord + Google. Voor de US-markt overwegen (in volgorde van ROI): **magic link / e-mail-OTP** (wachtwoordloos, laagste frictie — puur een Supabase-instelling, geen externe console, "lekker easy" — Khidr's voorkeur), **Microsoft/Azure** (Office 365-professionals), daarna optioneel **GitHub** (developer/RAG-hoek). **Apple** pas als er een iOS-app komt (App Store 4.8 vereist 't dan; web niet — kost Apple Developer-account + Services ID + key). Facebook/X overslaan. Per provider = provider-console-app + Supabase-config + een knop (het nieuwe gedeelde `GoogleSignInButton`-patroon maakt knoppen toevoegen makkelijk). NB Google OAuth-consent-scherm op "In production"/gepubliceerd zetten om het "unverified app"-waarschuwingsscherm weg te halen (Google Cloud Console, geen code).
