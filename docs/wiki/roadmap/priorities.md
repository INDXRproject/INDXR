...
- [ ] **1.26 — Schone-lei reset vóór launch** (financieel-kritiek, NÁ de reservering-fix uitvoeren) 💰
    ...
    **⚠️ GEEN FK-CASCADE:** geverifieerd 2026-07-09 — er is **geen enkele foreign-key van de public-tabellen naar `auth.users`**. `DELETE FROM auth.users` cascadet dus NIET; child-rijen (credit_transactions, jobs, user_credits, messages, …) moeten **expliciet** in FK-veilige volgorde verwijderd worden vóór de auth-rij, anders ontstaan orphans. De 1.26-reset-tooling moet dit expliciet doen.
