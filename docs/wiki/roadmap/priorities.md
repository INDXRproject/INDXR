...
- [ ] **1.27 — yt-dlp bot-detectie / read-timeout bij playlist-/video-extractie** (aparte proxy-reliability-track, niet in de stuck-playlist-fix opgelost)
    Waargenomen: in de live ADR-050-verificatie faalden **2/14** playlist-video's op yt-dlp bot-detectie ("Sign in to confirm you're not a bot"); later blokkeerde een yt-dlp **read-timeout** playlist `bfd1d7ed` (28 video's, stuck op 19/28). **Los van credits** — gefaalde video's worden correct niet gesetteld en meegerefund. **ADR-051 maakt zulke hangs nu ONSCHADELIJK/HERSTELBAAR** (per-video-timeout + reap-pass), maar lost de onderliggende bot-detectie/read-timeout-FREQUENTIE NIET op — dat blijft deze track. Proxy/yt-dlp-kwestie (Decodo residentieel, PO-tokens/bgutil, `web_embedded`-client). Aanpakken via cookies/PO-token-strategie, proxy-rotatie of een retry-tier. Zie [ADR-007](../decisions/007-bgutil-pot.md) en [ADR-051](../decisions/051-stuck-running-playlist-recovery.md).
...
- [x] **1.28 — Stuck-running-playlist recovery** ✅ 2026-07-09 (ADR-051, financieel-kritiek)
    ...
    **OPENSTAAND (nog niet beslist):** de eerder gekozen **Policy K** (retry-/retry-all-jobs resetten de free-tier `idx<3` → eerste ≤3 caption-video's van een retry-subset opnieuw gratis; kleine revenue-lek, consistent met per-video-retry). Bewust zo gelaten om de settle-logica niet te raken; herzien vóór launch of Policy S (retries onderdrukken de free-tier) gewenst is.
