...
- [ ] **F5b — `snapshot_finance_day` net-model loopt achter op ADR-063.** De nachtelijke snapshot berekent `net_profit_measured` nog met **volle gemeten COR** (niet against-revenue) **en de volle Stripe-fee bij de sale** (niet gedefereerd). De live-tab (`admin_finance_summary`) gebruikt sinds ADR-063 against-revenue + fee-defer. Gevolg: de Trend-net wijkt af van de headline-net met o.a. `deferred_fee`. De functie leest alle nog-bestaande `_geld_scope`-keys → geen crash, maar het net-model moet gelijkgetrokken (against-revenue + `recognized_fee` i.p.v. volle COR + volle fee). Raakt ook F5 (her-snapshotten).
