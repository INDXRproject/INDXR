...
- [ ] **Docs-gat: opslaglimiet (max + handhaving) ontbreekt in Limits** (gerapporteerd 2026-08-02, FASE-3 how-indxr-works-taak) — de **buy-space**-kant (basis 100 MB/account + 100 credits = +100 MB permanent) staat wél in rendered content op [`/docs/account/credits`](https://indxr.ai/docs/account/credits). Maar de **max-cap (500 MB)** en de **handhaving** (ADR-078: een nieuw transcript wordt geblokkeerd zodra de bibliotheek vol is → 413, géén credits kwijt) staan **nergens** in de rendered docs, en zeker niet op `/docs/reference/limits` waar een gebruiker die grens zou zoeken. Dit is een echt gat: de limiet wordt afgedwongen en een gebruiker kan er tegenaan lopen zonder uitleg. **Hoort in Limits** (met een kruislink naar Credits voor het bijkopen). Alleen genoteerd — in de FASE-3-taak bewust niet geschreven (die pagina legt het systeem uit, niet de limieten).
- [~] **`docs/content/*.md`-mirrors afgebouwd (drift-bron) — grotendeels klaar 2026-08-03.** Uitgevoerd: (1) de 6 **wees-drafts** zonder gerenderde route — `ARTIKEL-alternative-{downsub,happyscribe,notegpt,tactiq,turboscribe}.md` + `ARTIKEL-youtube-transcript-generator.md` — **verwijderd** (goedkeuring Khidr): de vergelijkingsdrafts botsen met [ADR-037](../decisions/037-no-comparison-pages.md) (geen comparison pages) en met het campagnebesluit in [keyword-demand-2026-08.md](../business/keyword-demand-2026-08.md) (niet bieden op concurrentmerken); de generator-draft dekt een intentie die `/transcribe` al bedient. (2) De **echte mirrors waarvan de gerenderde `.tsx` de inhoud volledig dekt** (per-bestand geverifieerd via subagent-vergelijking) — 10 stuks — **verwijderd**: `ARTIKEL-{blog-youtube-channel-knowledge-base, blog-youtube-transcripts-vector-database, bulk-youtube-transcript, youtube-age-restricted-transcript, youtube-members-only-transcript, youtube-playlist-transcript, youtube-srt-download, youtube-transcript-markdown, youtube-transcript-obsidian, youtube-transcript-without-extension}.md`. **NOG OPEN — 6 mirrors met een echte gap** (de gerenderde pagina dekt niet alles; bewust laten staan tot iemand port-of-dropt): **`ARTIKEL-blog-chunk-youtube-transcripts-for-rag.md`** (FAQ "chunk at extraction vs export time" — minor); **`ARTIKEL-youtube-transcript-csv.md`** (merged/playlist-CSV-sectie + `video_id`/`video_title`-kolommen — de `.tsx` verving dit bewust door "ZIP met losse CSV's"; de `.md`-claim is dus vermoedelijk stale → waarschijnlijk droppen); **`ARTIKEL-youtube-transcript-for-rag.md`** (LlamaIndex-codevoorbeeld ontbreekt op de `.tsx`); **`ARTIKEL-youtube-transcript-json.md`** ("Does This Match youtube-transcript-api?"-sectie + JSONL-FAQ + merged-JSON-FAQ); **`ARTIKEL-youtube-transcript-not-available.md`** (FAQ "search binnen transcript geeft geen resultaten" — minor); **`PRICING-PAGE.md`** (concurrent-prijstabel Rev/HappyScribe/TurboScribe/Otter — staat nergens gerenderd, maar botst met ADR-037; plus stale prijzen → waarschijnlijk droppen). Besluit per bestand (Khidr): content porten naar de `.tsx` of de gap bewust laten vallen en het bestand alsnog verwijderen. `ARCHITECTURE.md` blijft — echte docs, geen page-mirror. `product-truth.md` bijgewerkt (sync-regel geldt nu alleen nog voor deze 6).
