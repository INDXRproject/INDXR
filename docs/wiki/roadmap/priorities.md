...
- [ ] **1.34 — Graceful worker-drain bij deploy + watchdog-resume van interrupted jobs** — **post-launch (deploy-hygiëne)**
  **Werkregel nu (geen code):** een deploy naar de **worker**-service herstart de container en **doodt elke lopende job** (transcriptie/playlist) — er is geen graceful drain. De watchdog re-enqueuet `interrupted`/stuck jobs pas bij de volgende cron-pass (elke 2 min; Pass 1 re-enqueue, ADR-049/051) → recovery is **vertraagd, niet instant**, en single-video jobs verliezen hun in-progress werk. **Regel: niet naar `master` pushen terwijl er actieve jobs lopen** (check niet-terminale `transcription_jobs`+`playlist_extraction_jobs` / `ActiveJobsIndicator`). Zie [operations/deployment.md](../operations/deployment.md#zero-downtime-deploy-health-gated-cutover). **Toekomst (post-launch):** (a) graceful worker-drain — vang `SIGTERM`, stop met nieuwe jobs oppakken, laat lopende jobs afronden binnen Railway's shutdown-grace (ARQ ondersteunt job-completion op shutdown); (b) de watchdog-resume-latency verkleinen of een deploy-hook die wacht tot de queue leeg is. De API-service is al zero-downtime (health-gated cutover via `backend/railway.json`); dit item betreft alleen de worker. Koppelt aan 1.23 (worker-concurrency) en ADR-049/051.
