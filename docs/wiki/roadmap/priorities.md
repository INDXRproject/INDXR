...
- [ ] **`docs/content/*.md`-mirrors afbouwen (drift-bron) — gestart 2026-08-03, uploadformaten-taak.** Deze static-mirror-map bevat TWEE soorten bestanden, en dat onderscheid is cruciaal: **(a) echte mirrors** — een `ARTIKEL-*.md`/`LANDING-PAGE.md`/`PRICING-PAGE.md` waarvan een gerenderde `.tsx`-pagina bestaat (`/articles/*`, `/`, `/pricing`, `/docs/how-indxr-works`). Die zijn een niet-gerenderde tweede waarheid → **verwijderen** (de `.tsx` is de bron; git houdt de historie). In de uploadformaten-taak zijn de 4 aangeraakte mirrors al weg (`ARTIKEL-audio-to-text`, `ARTIKEL-how-it-works`, `ARTIKEL-youtube-to-text`, `LANDING-PAGE`). **Nog te verwijderen** (ongewijzigd gelaten — chirurgische scope): `ARTIKEL-{blog-chunk-youtube-transcripts-for-rag, blog-youtube-channel-knowledge-base, blog-youtube-transcripts-vector-database, bulk-youtube-transcript, youtube-age-restricted-transcript, youtube-members-only-transcript, youtube-playlist-transcript, youtube-srt-download, youtube-transcript-csv, youtube-transcript-for-rag, youtube-transcript-json, youtube-transcript-markdown, youtube-transcript-not-available, youtube-transcript-obsidian, youtube-transcript-without-extension}.md` + `PRICING-PAGE.md`. Verifieer per bestand dat de gerenderde pagina de content dekt (wiki noemt ze letterlijke mirrors) vóór verwijderen. **(b) Wees-drafts (GEEN mirror, sole source)** — `ARTIKEL-alternative-{downsub,happyscribe,notegpt,tactiq,turboscribe}.md` + `ARTIKEL-youtube-transcript-generator.md` hebben **geen** gerenderde route (`/alternative/*` en `/youtube-transcript-generator` bestaan niet, worden nergens gelinkt). Dat zijn drafts voor nooit-gebouwde pagina's → **niet verwijderen zonder besluit**; ze dragen bovendien nog **verouderde tellingen** (7-uploadformaten i.p.v. 9, en "8 exportformaten" i.p.v. 7). Besluit: bouwen (dan uit de single sources renderen) of schrappen. `ARCHITECTURE.md` blijft — echte docs, geen page-mirror. Werk bij afronding `product-truth.md` bij (de "houd mirrors bij de hand gelijk aan models.ts"-regel vervalt zodra de mirrors weg zijn).
