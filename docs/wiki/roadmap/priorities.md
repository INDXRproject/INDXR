...
- [ ] **1.38 — Onboarding-gate laat checkout-intent vallen (pricing → auth → billing)** — **UX-funnel, niet-blocker**
  Sinds 2026-07-15 stuurt een marketing-koopknop uitgelogde users naar `login?next=https://app.indxr.ai/dashboard/billing?checkout=<plan>` en na inloggen land je op billing met auto-checkout — **maar** `loginAction` (`packages/shared/src/actions/auth-actions.ts:67`) én `auth/callback/route.ts:66` redirecten users met `onboarding_completed=false` **onvoorwaardelijk** naar `/onboarding`, vóór ze het `next`/`redirectTo`-doel honoreren; de onboarding-pagina (`apps/marketing/src/app/onboarding/page.tsx:46`) redirect na afronden hardcoded naar `${APP}/dashboard`. Netto: een **nieuwe** signup die op een pakket klikt, verliest na onboarding het gekozen pakket (belandt op dashboard i.p.v. billing-checkout). Voor **bestaande/ge-onboarde** users werkt de volledige funnel wél (geverifieerd 2026-07-15: login → billing → Stripe met correct pakket). Fix (buiten scope van de knop-taak, raakt de onboarding/auth-flow): thread het doel door — `/onboarding?next=<target>` in beide redirects + onboarding-completion honoreert `next` i.p.v. hardcoded `/dashboard`, met dezelfde host-validatie (`app.indxr.ai`) tegen open-redirect. Launch-relevantie: **UX-funnel** (checkout-conversie voor net-geregistreerde kopers), geen blocker. (Gemeld 2026-07-15.)
