...
- [ ] **TEST 4 — Stripe net-capture:** doe één **echte** test-betaling op de **kleinste tier** (Try). Verifieer daarna in de `credit_transactions`/purchase-rij dat de `metadata` de netto-velden bevat: **`amount_tax` (BTW) + `stripe_fee` + `net_settlement` (+ `settlement_currency`)** én `kind='purchase'`. Dit is de enige live-only capture (best-effort `PaymentIntent→Charge→BalanceTransaction`, ADR-054); DB-schema + code zijn groen maar een echte betaling is nooit geverifieerd.
