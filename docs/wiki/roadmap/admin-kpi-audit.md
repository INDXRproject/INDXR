...
- **Fase 2 — Operations verdiepen** (deels ✅ 2026-07-26). **Error-CAPTURE thoughtfulness (kern):** de rijke ~11-slug-taxonomie werd voor standalone jobs weggegooid vóór de DB-write — 13 branches in `transcription_pipeline.py` schreven `error_type=NULL` → "uncategorized". Nu persisteren álle branches hun label (api_error/no_speech/members_only/internal_error/credit-/validation-/duration-/compression-error); watchdog-reaper → `stuck_pending`/`worker_crashed`; classifier uitgebreid met `proxy_error`, `ytdlp_parse`, meer youtube_restricted/timeout-keywords (15/15 real-case-test). Dashboard `ERROR_META` bijgewerkt + **info-tooltips** ("?" met definitie, o.a. activation = "spent credits incl. gratis welcome-credits"). **Nog open in Fase 2:** trend per dag, verwerkingstijd p50/p95 + caption-vs-AI split, error-drill-down naar ruwe `error_message`. **Follow-ups (backend):** caption-failures in `usage_logs` dragen nog geen `error_type` (RPC-uitbreiding); `main.py` inline caption-classifier is stale kopie (dedupe naar shared); `api_error` splitsen in rate-limit/server/timeout.
...
- **Weergave gebouwd (2026-07-25):** RPC `admin_reversals_summary` + Finance-kaart **"Refunds & chargebacks"** (scope/periode-aware): refunds, chargebacks (lost/open-at-risk/won/fees), "Money out (settled)", "Revenue after reversals". Aparte RPC → de geauditeerde `admin_finance_summary` blijft ongemoeid. Lege staat live (0 reversals); vult vanzelf bij de eerste echte refund/chargeback.
- **Openstaand:** (a) ✅ **Khidr heeft de drie event-types aangezet** op de live webhook-endpoint (2026-07-25). (b) **Follow-up (optioneel):** de reversals verrekenen in de P&L-hero (`admin_finance_summary`) i.p.v. alleen de losse kaart. Geen dataverlies meer — kan later over bewaarde data.
