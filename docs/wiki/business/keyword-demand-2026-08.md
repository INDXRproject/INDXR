...
- **[x] `how-to-get-a-youtube-transcript` — vraagvormen opgenomen, geen apart artikel (2026-08-26).** Discover groep 1: de vraagvorm staat op **index 51, bod €1,60, vijf formuleringen van 10K–100K**. Besluit uitgevoerd: het artikel is NIET teruggebracht (de schrapgrond — opbokssen tegen Google's eigen documentatie — staat nog), maar de vraagvormen zijn als H2 + FAQ ingepast waar ze inhoudelijk kloppen. `youtube-transcript-not-available`: nieuwe H2 "How to get a transcript of a YouTube video" (directe-antwoord-sectie boven de troubleshooting) + een FAQ met dezelfde vraag als eerste item. `quickstart`: nieuwe H2 "How to get a transcript of a YouTube video" met een korte directe-antwoord-alinea vóór de genummerde stappen; die pagina heeft geen FAQ-sectie, dus daar is de FAQ-vorm niet geforceerd. Alle vijf formuleringen (transcript/script × of/from) én de tweede laag (`download`/`copy transcript from youtube`) zijn door het product beantwoordbaar — niets weggelaten.
