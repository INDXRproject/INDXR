...
- ⚠️ **De opgeslagen rate is de REGULAR (off-peak) rate.** DeepSeek voert per **medio juli 2026** peak-pricing in: tijdens **UTC 01:00–04:00 én 06:00–10:00** is elk billing-item **2× de regular rate**. Er is **bewust géén peak-logic gebouwd** — de samenvatting is flat 3 credits en de werkelijke kost blijft sub-cent, óók bij 2×. Deze caveat staat er alleen zodat het kost-cijfer eerlijk geïnterpreteerd wordt (in een peak-venster is de werkelijke DeepSeek-kost het dubbele van de tabelwaarde, nog steeds verwaarloosbaar t.o.v. 3 credits). Zelfde noot in `cost_config.notes` (rij `deepseek_eur_per_1k_*`).
