...
| `supported-formats` | **Reference (nieuwe 5e bucket — bewuste keuze, zie §3)** | **Input-formaat-cluster, SEO-zet** (~9.100 zoekopdr./mnd VS, competition-index 43–86 — [keyword-demand-2026-08.md](keyword-demand-2026-08.md); geen bestaande pagina mikte erop). Alle **15** accepteerde formaten (**8 audio / 7 video** uit `UPLOAD_EXTENSIONS`), per formaat wat het is + herkomst; 500MB / 10u; 1cr/min; RAG 1cr/10min; 99 talen; 50 welkomstcredits — **alle getallen tegen de code geverifieerd** (`uploadFormats.ts`/`limits.ts`/`pricing.ts`/`models.ts`). **Geen FAQPage JSON-LD** (FAQ als platte tekst); **geen og-image** (nog niet gerenderd — hero = auto-SVG). Inbound-links vanuit `audio-to-text` + `video-to-text` bij de formatentabel. | live (nieuw 2026-08-31) |
