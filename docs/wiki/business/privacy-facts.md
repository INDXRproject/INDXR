...
## Addendum 2026-07-20 — Terms-acceptatie + vastlegging bij checkout (ADR-069)

Verplicht acceptatie-vinkje vóór de betaling ("I agree to the Terms of Service and Privacy Policy", beide klikbaar naar de marketing-host). **Server-side afgedwongen**: de checkout-route weigert met `400 "Terms acceptance required"` zonder `termsAccepted: true` — dus onomzeilbaar voor directe POSTs én de marketing-auto-checkout (die niet meer stil doorredirect). Aanvaarding vastgelegd in de nieuwe RLS-tabel `terms_acceptances` (`user_id`, `accepted_at`, `terms_version` = `LEGAL_VERSION` "2026-07-20", `documents` `{terms,privacy}`, `stripe_session_id`, `plan`) + `session.metadata.termsVersion` als duurzame, aan de betaling gekoppelde kopie. `ON DELETE CASCADE` op `user_id` → account-verwijdering wist ook de consent-rij (persoonsgegevens-belofte); het aan-de-betaling-gekoppelde bewijs blijft bij Stripe. Buiten de finance-keten → audit blijft 31/0/0. Keuze vs. Stripe's `consent_collection`: die steunt maar één ToS-link, geen tweede klikbare Privacy-link (geverifieerd tegen de Stripe-docs). Zie [ADR-069](decisions/069-terms-acceptance-at-checkout.md).
