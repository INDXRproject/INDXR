  ...
  return (
    ...
    <div className="flex flex-col items-center">
      ...
      <section className="w-full min-h-screen flex flex-col items-center border-b border-[var(--border)] relative overflow-hidden bg-[var(--bg)] pt-[80px] lg:pt-[130px] xl:pt-[150px] 2xl:pt-[180px]">
        <div className="absolute inset-0 dark:bg-[radial-gradient(ellipse_80%_50%_at_50%_-10%,rgba(167,139,250,0.08)_0%,transparent_70%),var(--bg)] pointer-events-none"></div>
        <HeroImage />
        <div className="container px-4 text-center relative z-10 flex flex-col items-center">
          ...
          <div className="flex flex-col sm:flex-row items-center justify-center gap-4 w-full sm:w-auto">
            ...
            <Link href="/pricing" className="w-full sm:w-auto">
              <button className="px-8 py-3 rounded-lg font-medium text-base bg-transparent border border-border text-fg hover:bg-surface-elevated transition-all duration-150 ease-out cursor-pointer h-12 w-full sm:w-auto">
                View Pricing
              </button>
            </Link>
          </div>
        </div>
        <p className="absolute bottom-6 left-0 right-0 text-center text-xs text-fg/40 dark:text-fg/60 z-10 px-4">
          No account needed — free for captioned videos. Sign up for credits, exports &amp; library access.
        </p>
      </section>

      {/* Features Grid */}
      <section className="w-full py-16 sm:py-24 flex justify-center border-b border-[var(--border)] bg-[var(--bg)]">
      ...
      <section className="w-full py-16 sm:py-24 flex justify-center border-b border-[var(--border)] bg-[var(--bg)]">
        <div className="container px-4">
          <h2 className="text-3xl sm:text-4xl font-bold tracking-tight text-center mb-12 text-[var(--fg)]">
      ...
      {/* Bottom CTA Section */}
      <section className="w-full py-16 sm:py-24 flex justify-center bg-[var(--bg)]">
        <div className="container px-4 text-center">
          <h2 className="text-4xl sm:text-5xl font-bold tracking-tighter mb-6 text-[var(--fg)]">
            Start with any YouTube video. It&apos;s free.
          </h2>
          <p className="text-lg sm:text-xl max-w-[640px] mx-auto mb-10 leading-relaxed text-[var(--fg-subtle)]">
          ...
          <p className="mt-4 text-sm text-[var(--fg-muted)]">
            <Link href="/how-it-works" className="hover:text-[var(--fg)] transition-colors underline underline-offset-2">
              How it works
            </Link>
          </p>
        </div>
...
function PersonaCard({ emoji, label, body }: { emoji: string; label: string; body: string }) {
  return (
    <div className="p-6 rounded-xl border bg-[var(--surface)] border-[var(--border)]">
      ...
      <h3 className="text-lg font-bold mb-2 text-[var(--fg)]">{label}</h3>
...
function TestimonialCard({ quote, name, role }: { quote: string; name: string; role: string }) {
  return (
    ...
    ">
      <p className="text-[15px] italic leading-relaxed text-[var(--fg-subtle)] mb-6 flex-grow">
        "{quote}"
      </p>
      <div>
        <div className="text-sm font-semibold text-[var(--fg)]">{name}</div>
        <div className="text-xs text-[var(--fg-muted)]">{role}</div>
      </div>
    </div>
  )
}
